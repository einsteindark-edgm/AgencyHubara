...
### L-26 · una falla de un nodo IO OPCIONAL (narrativa LLM) no debe tumbar el deliverable DETERMINISTA — y `localhost` es un default dev que revienta multi-caja con `[Errno 99]` (2026-06-23, primer run real del LLM en prod AWS)
- **Síntoma:** el operador corrió un workflow COMPLETO de GraphAgents en AWS y el nodo `ctwa_report` FALLÓ con `<urlopen error [Errno 99] Cannot assign requested address>`. El reporte entero (la tabla de unit-economics + el verdict) se perdió por una llamada de red — la PRIMERA salida saliente que hizo el reporter (la metió L-25). Tests verdes en local; feature rota en prod (la cara de red del gotcha #1).
- **Causa (DOS, independientes):** (1) **Robustez:** `run()` llamaba `narrate(input, llm=llm)` SIN try/except → cualquier excepción del LLM (proxy caído, timeout, respuesta malformada) propagaba y mataba el nodo, llevándose el render determinista que NO toca red (viola el espíritu de G-DET: puro vs nodo-IO marcado). (2) **Topología:** el stack prod de GraphAgents (`infra/compose/graphagents/`) corre en su propia EC2 con SOLO `postgres`+`agentspan`+`graphagents` — NO hay servicio LiteLLM ahí; el proxy `:4000` (con las keys) vive en la caja del backend principal. `LITELLM_PROXY_URL` no está seteada → `LiteLLMProxy` cae al default `http://localhost:4000` → desde el contenedor de GraphAgents `localhost` resuelve a `::1` (IPv6 no asignado al `lo` en imágenes slim) → `[Errno 99]` (no `111` connection-refused, que sería IPv4 sin listener). El nodo narrativo NUNCA pudo alcanzar el proxy: está en otra caja.
- **Fix:** (a) **Código (este incremento):** `run()` envuelve `narrate()` en try/except — el reporte SIEMPRE se emite; si el LLM falla, `narrative` = marcador VISIBLE ("no disponible — el modelo no respondió") + `narrative_error` con el error completo (trace/observabilidad). No se traga: se reubica de fatal a diagnóstico, igual que `narrative_invented` reubica una alucinación. El workflow COMPLETA. (b) **Infra (del operador):** para recuperar la narrativa en AWS, `LITELLM_PROXY_URL` debe apuntar a un proxy alcanzable desde la caja de GraphAgents (cross-box SG-a-SG al proxy del backend, o un LiteLLM dedicado en el stack de GraphAgents). El default `localhost` es SOLO dev.
- **Regla para el skill:** un nodo marcado (LLM/IO) es best-effort por diseño — su falla degrada VISIBLE, nunca tumba el deliverable determinista del `run()` puro. Envolvé toda llamada a un port que pueda fallar (red) en try/except que (1) preserva el output determinista, (2) deja un marcador client-facing, (3) registra el error crudo en un campo `*_error` para el trace. Y todo default `localhost`/`127.0.0.1`/`:port` en un vendor es un footgun multi-caja: documentá que en deploy distribuido la URL va por env al endpoint alcanzable. `[Errno 99]`=destino no-conectable (`0.0.0.0`/`::1`-no-asignado/sin-ruta); `[Errno 111]`=hay ruta pero nada escucha — el errno te dice si es topología o si el servicio está caído.
- **Guard:** `tests/graphs/test_ctwa_report_narrate.py::test_run_survives_an_unreachable_llm_proxy` (port que levanta el MISMO `URLError(OSError(99,...))` → run emite reporte + marcador + `narrative_error`, no crashea). Reproduce el crash de prod exacto.

### L-27 · un agente standalone entry-point NO aparece como Workflow en Acktos Studio si nadie lo declara raíz — `exposes_as_tool` es el bit que separa "componente de un pod" de "pod en sí mismo" (2026-07-10, window-strategist sin workflow en el catálogo)
- **Síntoma:** el operador buscó el workflow de `window-strategist` en el grupo Workflows de Acktos Studio y no existía. El agente estaba VIVO (C2, corriendo en prod cada 45 min vía Temporal Schedule → SSM), visible bajo "Agentes", con su caso en "Cases" — pero sin entrada operable como flujo.
- **Causa:** el grupo Workflows (`vscode-hubara/src/trees/catalogTree.ts → workflows()`) solo listaba raíces con aristas `agent` SALIENTES — supervisores de taskgraphs. Un analyzer standalone (solo `tools:`, cero sub-agentes) nunca calificaba, aunque `sdk/graph.py:execution_plan` ya lo trataba como flujo de un paso (`role: single`). El bit que distingue los dos roles YA estaba en los manifests: TODOS los componentes del pod ads declaran `exposes_as_tool: true`; window-strategist es el único `false` — entry-point que dispatcha un sistema externo (hubara), no pieza de composición.
- **Fix:** `workflows()` ahora lista (a) supervisores raíz (regla original) ∪ (b) standalone entry-points: `kind=agent`, sin supervisor entrante, `exposes_as_tool === false`. Con el payload real: antes 3 raíces (los 3 taskgraphs), ahora + `window-strategist` (clausura de 2 nodos: agente + parse-conversations). `greeter` (demo, `exposes_as_tool: true`) sigue afuera; los componentes siguen bajo el workflow de su supervisor.
- **Regla para el skill:** al crear un agente decidí SU ROL y declaralo en el manifest: **componente** (otro agente lo compone) → `exposes_as_tool: true` + referencialo desde su taskgraph en el mismo incremento; **entry-point** (lo dispatcha un sistema externo — hubara, un cron, un humano) → `exposes_as_tool: false` (el default) y verificá que aparece en Workflows de Acktos Studio. Un agente de producción que no es ninguna de las dos cosas está INVISIBLE para el operador — el guard lo caza. La definición de HECHO de un agente nuevo incluye: caso de ejecución (2.3b) **y** presencia operable en el Studio.
- **Guard:** `tests/architecture/test_workflow_visibility.py` — espejo documentado del filtro TS + invariante "ningún agente de producción (sin `group:`) queda invisible: o supervisado o raíz" + caso negativo sintético (standalone con `exposes_as_tool: true` = invisible). Si tocás el filtro en `catalogTree.ts`, actualizá el espejo en el MISMO cambio.

### L-28 · el pod de ads recomendaba escalar con un retorno inflado (2026-09-25, caso Halloween)
- **Síntoma:** el run `run-143a22615046` dijo `scale_budget` con MER 3.05 para la campaña Halloween. Con las ventas atribuidas a sus chats y confirmadas, el retorno era 1.43 (2.15 si se confirmaban 2 pedidos). Además, marcó `rotate_creative` en días sin ningún desgaste real y no podía decir QUÉ anuncio tocar.
- **Causa raíz:** tres huecos en el seed + el blend: (1) el central mandaba TODA la cuenta (Halloween + Amor y Amistad) aunque el run estuviera etiquetado con una campaña; (2) `manual_sales` eran TODAS las órdenes pagadas de la tienda, fechadas en UTC; (3) `merge-by-date` hace INTERSECCIÓN → los días con gasto y sin ventas desaparecían del cálculo (9 de 15). Y el diagnóstico era por día de CUENTA: sin granularidad segmento/anuncio/tarjeta.
- **Fix aplicado:** central (`ads`): `analysis-input?campaign_id=&from=&to=` → insights filtrados, ventas CONFIRMADAS de los chats de la campaña por día de Bogotá con días en cero, y `campaign_breakdown` (anuncio → segmento, Meta del periodo + diaria, chats con headline + estado/monto del pedido). Pod: agente `ctwa-scorecard` (tool `entity-economics`) con decisión por segmento/anuncio/tarjeta, CBO-aware y con mínimos de muestra; `numbers-qa` reconcilia el scorecard (entidades + jerarquía); `ctwa-report` explica (veredicto + "qué hacer, en orden") y la narrativa LLM recibe hallazgos, no la tabla diaria.
- **Regla para el skill:** un análisis de pauta se juzga con ventas ATRIBUIDAS y CONFIRMADAS del mismo alcance que el gasto (misma campaña, misma zona horaria), con los días sin venta en cero. Nunca blendear gasto de cuenta contra ventas de tienda, y nunca dejar que una intersección de fechas saque días del cálculo en silencio. Un veredicto que no nombra el anuncio/segmento a tocar no es accionable.
- **Guard:** `tests/graphs/test_ctwa_scorecard_golden.py` + el caso `fixtures/cases/halloween-flujo.case.yaml` (datos reales de prod, golden pineado): veredicto `hold_budget`/`pending_confirmation`, tarjeta «Chatea con nosotros», QA reconcilia. Central: `hubara_agency/tests/plugins/ads/test_meta_oauth_routes.py::test_analysis_input_de_una_campana_*` y `test_analysis_seed.py`.
