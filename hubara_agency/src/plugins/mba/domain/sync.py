...
_NATURAL_KEY = {"faqs": "question", "skills": "title", "connector": "name", "connector_tools": "name", "ui_skills": "title"}
_COMPARE_FIELDS = {
    "faqs": ("question", "answer"),
    "skills": ("title", "description", "skill"),
    "connector": ("name", "description", "base_url", "auth_type"),
    "connector_tools": ("name", "description", "request_definition", "user_auth_required"),
}
...
class RemoteState:
    ...
    def empty(cls) -> RemoteState:
        return cls(settings=None, business_info={}, faqs=(), skills=(), connectors=(), tools={}, ui_skills=())
...
def _redacted(op: SyncOp) -> SyncOp:
    ...
    api_key = body.get("auth_config", {}).get("api_key", {}) if isinstance(body.get("auth_config"), dict) else {}
...
def _blockers(cfg: MbaConfigDTO, by_section: Mapping[str, list[tuple[str, dict[str, Any]]]], api_key: str) -> tuple[str, ...]:
    blocked: list[str] = []
    ...
    for section in MANAGED_SECTIONS:
        for _, body in by_section.get(section, []):
            for s in _strings(body):
                found.update(m for m in _PLACEHOLDER.findall(s) if m != CONNECTOR_API_KEY_PLACEHOLDER)
...
def _resolve_api_key(body: dict[str, Any], api_key: str) -> dict[str, Any]:
    ...
    api = body.get("auth_config", {}).get("api_key", {}) if isinstance(body.get("auth_config"), dict) else {}
    for where in ("headers", "query_params", "body_params"):
        for entry in api.get(where, []) or []:
            if isinstance(entry, dict) and entry.get("value") == CONNECTOR_API_KEY_PLACEHOLDER:
                entry["value"] = api_key
    return body
...
def _collection(
    ...
    *,
    ...
    for label, body in wanted:
        ...
        differs = any(not _same(body.get(f), remote.get(f)) for f in fields if f in body)
        ops.append(SyncOp(section, label, "update" if differs else "noop", body if differs else {}, rid, "" if differs else "unchanged", **extra))
...
def _ui_skills(wanted: list[tuple[str, dict[str, Any]]], remote_items: Iterable[dict[str, Any]], managed: Mapping[str, str]) -> list[SyncOp]:
    ...
    for label, body in wanted:
        ...
        if not _same(body.get("component_type"), remote.get("component_type")):
            ops.append(SyncOp("ui_skills", label, "replace", body, rid, "component_type_changed"))
            continue
...
def build_plan(
    ...
    *,
    ...
    ops.extend(_collection("faqs", by_section.get("faqs", []), remote.faqs, managed_ids.get("faqs", {})))
    ops.extend(_collection("skills", by_section.get("skills", []), remote.skills, managed_ids.get("skills", {})))
    ...
    for label, _ in connectors:
        ...
        remote_tools = remote.tools.get(con_op.remote_id, ()) if con_op.remote_id else ()
        ops.extend(
            _collection(
                "connector_tools",
    ...
    ops.extend(_ui_skills(by_section.get("ui_skills", []), remote.ui_skills, managed_ids.get("ui_skills", {})))
    ...
    for label, body in by_section.get("settings", []):
        ...
        phrases_changed = phrases is not None and sent_hashes.get("settings", {}).get("never_say_phrases") != body_hash(phrases)
        remote_settings = remote.settings or {}
        fields_changed = remote.settings is None or any(
            not _same(send.get(f), _project(remote_settings, {f: send[f]}).get(f) if isinstance(send.get(f), dict) else remote_settings.get(f))
            for f in _SETTINGS_FIELDS
            if f in send
        )
        if fields_changed or phrases_changed:
            ops.append(SyncOp("settings", label, "update", send, reason="phrases_changed" if phrases_changed and not fields_changed else ""))
        else:
    ...
    fingerprint = body_hash([asdict(_redacted(op)) for op in ops] + [list(blocked), cfg.entity_id])
    return SyncPlan(agent_id=cfg.agent_id, entity_id=cfg.entity_id, ops=tuple(ops), blocked=blocked, fingerprint=fingerprint)
