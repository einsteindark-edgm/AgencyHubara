...
rate limit por (IP, tool) → 429 · API key → 503/401 · tool/método → 404/405 ·
tope de body → 413 · JSON → 400 · validación contra el ``request_definition``
autorado → 422 · lógica → 200 · tool sin lógica todavía → 501.

El teléfono que inyecta Meta (``customer_phone``) es la única identidad del
cliente: se convierte en ``session_key = wa_<dígitos>`` y toda lectura queda
acotada a esa sesión.
"""
