...
_GHOSTING_PROMPT = (
    ...
    "dirección/teléfono/pago por texto). Si el cliente NUNCA dijo que sí "
    "(solo eligió producto, preguntó, o aplazó con 'luego' / 'voy en "
    "camino'), NO es este caso aunque hayas mandado el formulario: es "
    "INTERESADO (el sistema degrada la etiqueta si no hay confirmación "
    "registrada). **Cuando SÍ confirmó, usa "
    ...
    "recibe) y el cliente se fue, usa `CONFIRMADO_SIN_DATOS` + "
    "`escalate_to_human(reason_category='ORDER_PENDING_SHIPPING_DETAILS')` "
    "con ese `error` en el summary. "
