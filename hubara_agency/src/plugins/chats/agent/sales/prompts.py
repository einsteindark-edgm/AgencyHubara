...
_GHOSTING_PROMPT = (
    ...
    "agente fue pedir datos de envío y nunca llegaron. **En este caso usá "
    "esta tag Y llamá también `escalate_to_human(reason_category="
    ...
    "envío + método de pago Y vos ya llamaste `register_order(...)` con "
    "éxito. Esta tag solo aplica si la conversación cerró limpia con venta "
    "registrada — si te llegó ghosting acá, lo más probable es que YA "
    "hayas etiquetado COMPRA_EXITOSA antes; en ese caso re-confirma con la "
    "misma tag.\n\n"
    "**REGLA DE ORO**: NO generes ninguna respuesta visible al usuario. SOLO "
    "llama la(s) herramienta(s) en silencio y termina. Para "
    "CONFIRMADO_SIN_DATOS llamás DOS tools: primero `manage_conversation_tag` "
