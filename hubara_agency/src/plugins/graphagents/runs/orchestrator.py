"""Orchestrator del buzón graphagents — convierte el POLL de Conductor en eventos del bridge.
...
async def poll_loop(run_id, execution_id, *, base_url, bus, interval=2.0, fetch=None, max_polls=900) -> None:
    ...
    from src.plugins.graphagents.runs import conductor
    ...
    for _ in range(max_polls):
        ...
        if event is not None:
            bus.publish("graphagents", event["type"], id=run_id, payload={"run_id": run_id, **event})
