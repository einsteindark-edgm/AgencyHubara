...
@pytest.mark.parametrize("trigger,facts,expected", [
    # Remarketing (INTERESADO) y el cliente responde: sí antes del pedido, no con orden registrada
    (ReleaseTrigger.REMARKETING_REPLY, HUBARA, ReleaseDecision(True, "remarketing_reply")),
    (ReleaseTrigger.REMARKETING_REPLY, ReleaseFacts("hubara", order_registered=True), ReleaseDecision(False, "order_in_progress")),
    ...
    (ReleaseTrigger.ETA_NOTICE, ReleaseFacts("hubara", order_registered=True), ReleaseDecision(True, "eta_notice")),
    # Handoff resuelto por el operador desde el inbox: sí
    (ReleaseTrigger.HANDOFF_RESOLVED, HUBARA, ReleaseDecision(True, "handoff_resolved")),
    (ReleaseTrigger.HANDOFF_RESOLVED, ReleaseFacts("hubara", order_registered=True), ReleaseDecision(True, "handoff_resolved")),
    # Comprobante verificado: no hasta emitir `agent_event` payment_received; luego sí
    (ReleaseTrigger.RECEIPT_VERIFIED, HUBARA, ReleaseDecision(False, "await_agent_event")),
    (ReleaseTrigger.RECEIPT_VERIFIED, ReleaseFacts("hubara", agent_event_emitted=True), ReleaseDecision(True, "receipt_verified")),
    # Manual (operador): sí
    (ReleaseTrigger.MANUAL, HUBARA, ReleaseDecision(True, "manual")),
])
def test_release_policy_table(trigger: ReleaseTrigger, facts: ReleaseFacts, expected: ReleaseDecision) -> None:
    assert decide_release(trigger, facts) == expected


@pytest.mark.parametrize("trigger", list(ReleaseTrigger))
def test_nothing_is_released_while_mba_already_controls_the_thread(trigger: ReleaseTrigger) -> None:
    assert decide_release(trigger, ReleaseFacts(control_owner="mba")) == ReleaseDecision(False, "already_mba")


@pytest.mark.parametrize("trigger", [t for t in ReleaseTrigger if t is not ReleaseTrigger.MANUAL])
def test_an_unknown_owner_blocks_automatic_releases_but_not_the_operator(trigger: ReleaseTrigger) -> None:
    ...
    assert decide_release(trigger, ReleaseFacts(control_owner=None)) == ReleaseDecision(False, "owner_unknown")
...
def test_triggers_are_stable_strings_for_the_http_contract() -> None:
    assert {t.value for t in ReleaseTrigger} == {
        "manual", "handoff_resolved", "remarketing_reply", "eta_notice", "receipt_verified",
    }
