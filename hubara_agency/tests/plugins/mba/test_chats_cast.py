...
async def test_session_action_forwards_with_the_service_token_to_the_contract_path(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, Any] = {}

    async def fake_forward(request: Any, method: str, path: str, **kw: Any) -> dict[str, Any]:
    ...
    out = await chats_cast.session_action("REQ", "wa_573001234567", "draft", {"aroma": "Lavanda"})
    assert out == {"ok": True}
    assert captured["request"] == "REQ" and captured["method"] == "POST"
    assert captured["path"] == "/api/chats/session-actions/wa_573001234567/draft"
    assert captured["auth"] == "service" and captured["base_url"] == "http://127.0.0.1:8000"
    assert captured["timeout"] == 12.5 and captured["cast_label"] == "mba→chats"
    assert captured["body"] == {"aroma": "Lavanda"}


async def test_session_key_is_percent_encoded_in_the_path(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, Any] = {}
