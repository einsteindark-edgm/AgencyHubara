...
def _seed(vault: Path, **extra: Any) -> None:
    FilesystemMetadataStore(vault).write(SESSION, {
        "tag": "COMPRA_EXITOSA", "active_route": "humano", "phone_number_id": "PHONE_777",
        "control_owner": "mba", "control_owner_updated_at_ms": NOW_MS - 60_000,
        **extra,
    })
...
def _uc(vault: Path, port: FakeAgentEvent | None = None, *, enabled: bool = True, allowed: bool = True,
        controls: bool = True, fallback_entity: str = "", now_ms: int = NOW_MS) -> tuple[EmitAgentEvent, FakeAgentEvent]:
...
def test_event_types_are_a_closed_catalogue_mapped_from_the_order_stages() -> None:
    assert set(AGENT_EVENT_TYPES) >= {"payment_received", "order_preparing", "order_ready", "order_shipped",
                                      "order_delivered", "order_cancelled", "episode_closed"}
    assert agent_event_type_for_stage("preparing", payment_confirmed=True) == "payment_received"
    assert agent_event_type_for_stage("preparing", payment_confirmed=False) == "order_preparing"
    assert agent_event_type_for_stage("ready", payment_confirmed=True) == "order_ready"
    assert agent_event_type_for_stage("shipping", payment_confirmed=False) == "order_shipped"
    assert agent_event_type_for_stage("delivered", payment_confirmed=True) == "order_delivered"
    assert agent_event_type_for_stage("cancelled", payment_confirmed=False) == "order_cancelled"
...
async def test_with_mba_in_front_the_event_goes_to_meta_and_is_recorded(vault: Path) -> None:
    _seed(vault)
    uc, port = _uc(vault)
    out = await uc.execute(SESSION, "order_shipped", order_id="order_1", message="Tu pedido #12 ya va en camino 🚚.",
                           payload={"stage": "shipping"}, source="eta")
    assert out == AgentEventOutcome(session_key=SESSION, emitted=True, reason="accepted", agent_event_id="fake-1",
                                    at_ms=NOW_MS)
    assert len(port.calls) == 1
    entity, to, event_type, description, payload = port.calls[0]
    assert (entity, to, event_type) == ("PHONE_777", f"+{CUSTOMER}", "order_shipped")
    assert description == build_description("order_shipped", "Tu pedido #12 ya va en camino 🚚.")
    assert payload == {"stage": "shipping", "order_id": "order_1"}
    m = _meta(vault)
    assert m["control_owner"] == "mba" and m["tag"] == "COMPRA_EXITOSA"  # nada más se toca
    assert m["agent_events"] == [{
        "type": "order_shipped", "order_id": "order_1", "at_ms": NOW_MS, "status": "accepted",
        "agent_event_id": "fake-1", "error": None, "source": "eta",
    }]
...
async def test_the_guards_fail_closed_and_nothing_is_called_nor_written(vault: Path, kw: dict, reason: str) -> None:
    _seed(vault)
...
async def test_an_unknown_session_or_key_or_type_is_refused(vault: Path) -> None:
    uc, port = _uc(vault)
    assert (await uc.execute(SESSION, "order_shipped", message="m")).reason == "session_unknown"
    assert (await uc.execute("573001234567", "order_shipped", message="m")).reason == "session_unknown"
    _seed(vault)
    assert (await uc.execute(SESSION, "whatever", message="m")).reason == "unknown_event_type"
    assert port.calls == []
...
async def test_the_same_event_for_the_same_order_is_emitted_once(vault: Path) -> None:
    ...
    assert (again.emitted, again.reason, again.agent_event_id) == (False, "already_emitted", "fake-1")
    # otro pedido, mismo tipo → evento nuevo; otro tipo, mismo pedido → evento nuevo
    assert (await uc.execute(SESSION, "order_shipped", order_id="order_2", message="m")).emitted is True
    assert (await uc.execute(SESSION, "order_delivered", order_id="order_1", message="m")).emitted is True
...
async def test_a_meta_rejection_is_recorded_and_does_not_block_a_retry(vault: Path) -> None:
    _seed(vault)
    uc, port = _uc(vault, FakeAgentEvent(fail_with=AgentEventError("rejected", status=400, detail="Unknown entity")))
    out = await uc.execute(SESSION, "order_shipped", order_id="order_1", message="m")
    assert out == AgentEventOutcome(session_key=SESSION, emitted=False, reason="rejected", agent_event_id=None,
                                    at_ms=NOW_MS, error="400 Unknown entity")
    assert _meta(vault)["agent_events"][-1] == {
        "type": "order_shipped", "order_id": "order_1", "at_ms": NOW_MS, "status": "rejected",
        "agent_event_id": None, "error": "400 Unknown entity", "source": None,
    }
    port.fail_with = None
    assert (await uc.execute(SESSION, "order_shipped", order_id="order_1", message="m")).emitted is True


async def test_an_ambiguous_timeout_counts_as_possibly_delivered_so_it_is_not_repeated(vault: Path) -> None:
    _seed(vault)
    uc, port = _uc(vault, FakeAgentEvent(fail_with=AgentEventError("ambiguous", detail="ReadTimeout: slow")))
    out = await uc.execute(SESSION, "order_shipped", order_id="order_1", message="m")
    assert (out.emitted, out.reason, out.error) == (False, "ambiguous", "ReadTimeout: slow")
    port.fail_with = None
    again = await uc.execute(SESSION, "order_shipped", order_id="order_1", message="m")
    assert (again.emitted, again.reason) == (False, "already_emitted")


async def test_the_entity_id_falls_back_to_the_configured_phone_number_id(vault: Path) -> None:
    _seed(vault, phone_number_id=None)
    uc, port = _uc(vault, fallback_entity="PHONE_FALLBACK")
    assert (await uc.execute(SESSION, "order_shipped", order_id="order_1", message="m")).emitted is True
    assert port.calls[0][0] == "PHONE_FALLBACK"
    uc, port = _uc(vault)
    assert (await uc.execute(SESSION, "order_delivered", order_id="order_1", message="m")).reason == "entity_id_missing"
    assert port.calls == []


async def test_the_audit_trail_is_capped(vault: Path) -> None:
    _seed(vault, agent_events=[{"type": "order_ready", "order_id": f"o{i}", "at_ms": i, "status": "accepted",
                                "agent_event_id": None, "error": None, "source": None} for i in range(AGENT_EVENTS_CAP)])
    uc, _ = _uc(vault)
    assert (await uc.execute(SESSION, "order_shipped", order_id="order_new", message="m")).emitted is True
    events = _meta(vault)["agent_events"]
    assert len(events) == AGENT_EVENTS_CAP and events[-1]["order_id"] == "order_new" and events[0]["order_id"] == "o1"
