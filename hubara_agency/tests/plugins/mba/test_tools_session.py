...
async def test_set_order_slot_forwards_the_slots_as_is() -> None:
    chats = _Chats(
        {"updated": True, "order_draft": {"producto": "luz-serena", "cantidad": "2"}}
    )
    out = await session.set_order_slot(
        chats, None, session_key=_S, params={"producto": "luz-serena", "cantidad": 2}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("anticipado", "transfer"),
        ("pago anticipado (Nequi)", "transfer"),
        ("Nequi", "transfer"),
...
async def test_register_order_maps_meta_params_to_the_chats_body_and_applies_rule_241() -> (
    None
):
    chats = _Chats(
        {
            ...
            "portavelas_handles": [],
            "episode_closed": {"episode_id": "ep", "closing_tag": "X"},
            "escalated": True,
    ...
    params = {
        "items": [
            {"handle": "luz-serena", "variant_label": "Lavanda, Blanco", "quantity": 2}
        ],
        "ciudad": "Bogotá",
        "barrio": "Chapinero",
        "direccion": "Cl 1 # 2-3",
    ...
    assert (
        out["registered"] is True
        and out["order_id"] == "order_1"
        and out["order_reference"] == "#22 (Luz Serena)"
    )
    assert (out["subtotal_cop"], out["shipping_cop"], out["total_cop"]) == (
        58000,
        7900,
        65900,
    )
    assert (
        out["shipping_is_minimum_rate"] is True and out["portavelas_included"] is False
    )
    assert out["payment_instructions_sent"] is True
    assert (
        "episode_closed" not in out and "escalated" not in out and "provider" not in out
    )  # interno de chats
    assert "registrado" in out["message"]


async def test_register_order_with_cash_on_delivery_never_returns_shipping_or_total() -> (
    None
):
    chats = _Chats(
        {
            ...
            "portavelas_handles": ["duo-zodiacal"],
            "payment_instructions_sent": False,
        }
    )
    params = {
        "items": [{"handle": "duo-zodiacal", "quantity": 1}],
        "ciudad": "Medellín",
        "direccion": "x",
        "telefono": "3001234567",
        "nombre_recibe": "Ana",
    ...
    assert (
        out["subtotal_cop"] == 58000
        and "shipping_cop" not in out
        and "total_cop" not in out
    )
    assert (
        "transportadora" in out["shipping_note"] and out["portavelas_included"] is True
    )
    assert out["already_registered"] is True


async def test_register_order_rejects_unknown_payment_method_without_calling_chats() -> (
    None
):
    chats = _Chats()
    out = await session.register_order(
        chats,
        None,
        session_key=_S,
        params={
    ...
    chats = _Chats(
        {
            "registered": False,
            "order_id": None,
            "error_detail": "unknown_product",
            "problems": ["unknown_product:x"],
        }
    )
    out = await session.register_order(
        chats,
    ...
    chats = _Chats(
        {
            ...
            "motivo": "m",
            "message": "ok",
            "episode_closed": {"episode_id": "e", "closing_tag": "RECHAZO"},
        }
    )
    out = await session.manage_conversation_tag(
        chats, None, session_key=_S, params={"tag": "RECHAZO", "motivo": "m"}
    )
    assert (
        chats.calls == [(_S, "tag", {"tag": "RECHAZO", "motivo": "m"})]
        and out["tag"] == "RECHAZO"
    )
    out = await session.manage_conversation_tag(
        chats, None, session_key=_S, params={"tag": "COMPRA_EXITOSA", "motivo": "m"}
    )
    assert (
        out["error"] == "invalid_tag"
        and out["accepted"] == ["INTERESADO", "RECHAZO"]
        and len(chats.calls) == 1
    )


async def test_manage_conversation_tag_tells_the_agent_when_hubara_applied_another_tag() -> (
    None
):
    ...
    out = await session.manage_conversation_tag(
        chats, None, session_key=_S, params={"tag": "interesado", "motivo": "m"}
    )
    assert chats.calls == [(_S, "tag", {"tag": "INTERESADO", "motivo": "m"})]
    assert (
        out["tag"] == "CONFIRMADO_SIN_DATOS"
        and out["proposed_tag"] == "INTERESADO"
        and out["reconciled"] is True
    )
    ...
    assert (
        ...
        and "pedido registrado" in out["message"]
        and "no vuelvas" in out["message"].lower()
    )
...
async def test_manage_conversation_tag_when_a_colleague_already_has_the_thread() -> (
    None
):
    chats = _Chats(
        exc=HTTPException(
            status_code=409,
            detail="already_human: un colega tiene la conversación; no se etiqueta",
        )
    )
    out = await session.manage_conversation_tag(
        chats, None, session_key=_S, params={"tag": "INTERESADO", "motivo": "m"}
...
async def test_escalate_to_human_validates_the_reason_category() -> None:
    ...
    out = await session.escalate_to_human(
        chats,
        None,
        session_key=_S,
        params={"reason_category": "BULK_ORDER", "summary": "30 uds"},
    )
    assert chats.calls == [
        (_S, "escalate", {"reason_category": "BULK_ORDER", "summary": "30 uds"})
    ]
    assert out["escalated"] is True
    out = await session.escalate_to_human(
        chats, None, session_key=_S, params={"reason_category": "OTHER", "summary": "x"}
    )
    assert (
        out["error"] == "invalid_reason_category"
        and "BULK_ORDER" in out["accepted"]
        and len(chats.calls) == 1
    )
...
async def test_cast_failures_become_explicit_error_envelopes(
    status: int, expected: dict[str, Any]
) -> None:
    ...
    out = await session.escalate_to_human(
        ...
        None,
        session_key=_S,
        params={"reason_category": "BULK_ORDER", "summary": "x"},
    )
