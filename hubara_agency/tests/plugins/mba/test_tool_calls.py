...
def test_contracts_come_from_the_authored_request_definitions(contracts) -> None:
    ...
    assert set(ro.required) == {"items", "ciudad", "direccion", "telefono", "nombre_recibe", "metodo_pago"}
    ...
    assert "customer_phone" not in ro.params  # el teléfono no es un parámetro del agente


def test_get_call_coerces_query_strings_keeps_empty_q_and_drops_unknown_params(contracts) -> None:
...
def test_missing_phone_bad_types_and_missing_required_are_reported_together(contracts) -> None:
...
def test_array_items_are_validated_against_their_object_schema(contracts) -> None:
    ...
    ok = parse_tool_call(
        c,
        {"customer_phone": _PHONE, "items": [{"handle": "vela", "quantity": 2, "variant_label": "Lavanda, Blanco", "x": 1}]},
    )
    assert ok.params["items"] == [{"handle": "vela", "quantity": 2, "variant_label": "Lavanda, Blanco"}]
    with pytest.raises(ToolCallError) as e:
    ...
    with pytest.raises(ToolCallError):  # bool NO es integer
        parse_tool_call(c, {"customer_phone": _PHONE, "items": [{"handle": "vela", "quantity": True}]})
    with pytest.raises(ToolCallError):
...
def test_oversized_strings_are_rejected(contracts) -> None:
    with pytest.raises(ToolCallError) as e:
        parse_tool_call(contracts["search_products"], {"customer_phone": _PHONE, "q": "x" * (MAX_STRING_LEN + 1)})
