...
def test_agents_and_config_are_served_from_the_authored_files(
    ...
    assert (
        agents[0]["entity_id"] == tenant_env["WHATSAPP_PHONE_NUMBER_ID"]
    )  # el número del tenant, desde el entorno
...
def test_connector_router_is_public_but_fails_closed_without_a_key(
    ...
    c = _client()
    assert (
        c.get("/api/mba/tools/search_products", headers={"X-API-Key": "x"}).status_code
        == 503
    )


def test_connector_tools_exist_for_every_declared_tool_and_require_the_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert (
        c.get(
            "/api/mba/tools/search_products", headers={"X-API-Key": "otro"}
        ).status_code
        == 401
    )
...
class _Catalog:
    async def search(self, q: str, *, limit: int = 10, category: str | None = None):
        ...
        return SearchResult(
            ...
            truncated=False,
            stale=False,
            manifest=CatalogManifestDTO(version="v", fetched_at="t", product_count=0),
            results=[],
        )
...
def test_read_tools_run_against_the_injected_ports(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    c = _client_with_deps(monkeypatch)
    ok = {"X-API-Key": "secreto"}
    r = c.get(
        "/api/mba/tools/search_products",
        headers=ok,
        params={"customer_phone": _PHONE, "q": "vela", "limit": "3"},
    )
    assert (
        r.status_code == 200
        and r.json()["query"] == "vela"
        and r.json()["results"] == []
    )
    r = c.get(
        "/api/mba/tools/list_categories", headers=ok, params={"customer_phone": _PHONE}
    )
    assert r.status_code == 200 and r.json() == {"count": 0, "categories": []}
    r = c.get(
        "/api/mba/tools/get_product_by_handle",
        headers=ok,
        params={"customer_phone": _PHONE, "handle": "nope"},
    )
    assert r.status_code == 200 and r.json()["found"] is False
    r = c.get(
        "/api/mba/tools/check_order_status",
        headers=ok,
        params={"customer_phone": _PHONE},
    )
    assert r.status_code == 200 and r.json()["orders"] == []
    r = c.post(
        "/api/mba/tools/verify_order_for_checkout",
        headers=ok,
        json={"customer_phone": _PHONE, "items": [{"handle": "x", "quantity": 1}]},
    )
    assert (
        r.status_code == 200 and r.json()["error"] == "catalog_unavailable"
    )  # sin verificador en este proceso
...
def test_write_tools_delegate_to_the_chats_cast_scoped_to_the_customer_phone(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert cast.calls == [
        ("wa_573001234567", "draft", {"producto": "luz-serena", "cantidad": 2})
    ]
    cast = _ChatsCast(
        {
            "escalated": True,
    ...
    c = _client_with_deps(monkeypatch, chats=cast)
    r = c.post(
        "/api/mba/tools/escalate_to_human",
        headers=ok,
        json={
    ...
    assert (
        r.status_code == 200
        and r.json()["escalated"] is True
        and cast.calls[0][1] == "escalate"
    )
    r = c.post(
        "/api/mba/tools/manage_conversation_tag",
        headers=ok,
        json={"customer_phone": _PHONE, "tag": "HUMANO", "motivo": "x"},
    )
    assert (
        r.status_code == 200
        and r.json()["error"] == "invalid_tag"
        and len(cast.calls) == 1
    )
    # el contrato de Meta sigue validando antes del cast: campo requerido ausente → 422, sin llamar a chats
    r = c.post(
        "/api/mba/tools/register_order",
        headers=ok,
        json={"customer_phone": _PHONE, "items": [{"handle": "x", "quantity": 1}]},
    )
...
def test_a_failing_cast_is_an_explicit_error_for_the_agent_not_a_500(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    c = _client_with_deps(
        ...
        chats=_ChatsCast(
            exc=HTTPException(
                status_code=502, detail="cast mba→chats: provider no disponible"
            )
        ),
    )
    r = c.post(
    ...
    assert (
        r.status_code == 200
        and r.json()["error"] == "chats_unavailable"
        and r.json()["applied"] is False
    )


def test_invalid_calls_are_422_with_the_list_of_problems_and_bad_json_is_400(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    c = _client_with_deps(monkeypatch)
    ok = {"X-API-Key": "secreto"}
    r = c.get(
        "/api/mba/tools/get_product_by_handle",
        headers=ok,
        params={"customer_phone": "abc"},
    )
    assert r.status_code == 422
    assert r.json()["error"] == "invalid_request"
    assert any("customer_phone" in e for e in r.json()["errors"]) and any(
        "handle" in e for e in r.json()["errors"]
    )
    r = c.post(
        "/api/mba/tools/verify_order_for_checkout",
    ...
    huge = (
        '{"customer_phone": "%s", "items": [{"handle": "x", "quantity": %s}]}'
        % (_PHONE, "9" * 5000)
    ).encode()
    r = c.post(
        "/api/mba/tools/verify_order_for_checkout",
...
def test_public_router_caps_body_size_and_rate_limits_per_ip_and_tool(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    c = _client_with_deps(monkeypatch)
    ok = {"X-API-Key": "secreto"}
    big = {
        "customer_phone": _PHONE,
        "items": [{"handle": "x" * 1500, "quantity": 1}] * 50,
    }
    ...
    r = c.post(
        "/api/mba/tools/verify_order_for_checkout", headers=ok, content=_chunks()
    )
    assert r.status_code == 413
    monkeypatch.setattr(
        connector, "_rate_limiter", RateLimiter(capacity=2, refill_per_s=0.0)
    )
    codes = [
        c.get(
            "/api/mba/tools/list_categories",
            headers=ok,
            params={"customer_phone": _PHONE},
        ).status_code
        for _ in range(3)
    ...
    assert (
        c.get(
            "/api/mba/tools/list_categories", headers={"X-API-Key": "otra"}
        ).status_code
        == 401
    )
...
def test_rate_limit_keys_on_the_forwarded_client_ip_and_bad_keys_have_their_own_small_bucket(
    ...
    ok = {"X-API-Key": "secreto"}
    monkeypatch.setattr(
        connector, "_rate_limiter", RateLimiter(capacity=1, refill_per_s=0.0)
    )
    ...
    monkeypatch.setattr(
        connector, "_bad_key_limiter", RateLimiter(capacity=2, refill_per_s=0.0)
    )
    bad = {"X-API-Key": "nope", "X-Forwarded-For": "3.3.3.3"}
    assert [
        c.get("/api/mba/tools/list_categories", headers=bad).status_code
        for _ in range(3)
    ] == [401, 401, 429]
    ...
    }  # otra IP con la key buena: su bucket general está intacto
    assert (
        c.get(
            "/api/mba/tools/list_categories",
            headers=meta,
...
def test_connector_key_check_is_constant_time_safe_with_non_ascii_and_does_not_leak_config(
    ...
    r = c.get(
        "/api/mba/tools/search_products",
        headers={"X-API-Key": "señuelo-ñ".encode("latin-1")},
    )
...
def test_connector_refuses_customers_outside_the_closed_list(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    r = c.get(
        "/api/mba/tools/search_products",
        params={"customer_phone": "+573001234567", "q": "vela"},
        headers={"X-API-Key": "secreto"},
    )
...
def test_session_control_is_served_from_the_session_metadata(
    ...
    assert c.get(f"/api/mba/sessions/{key}/control").json() == {
        "session_key": key,
        "control_owner": None,
        "control_owner_since_ms": None,
        "control_owner_updated_at_ms": None,
        "control_owner_app_id": None,
        "thread_control": None,
        "history": [],
    }
    ...
    store.write(
        ...
        {
            "control_owner": "mba",
            "control_owner_since_ms": 1,
            "control_owner_updated_at_ms": 2,
            "control_owner_app_id": "APP_MBA",
    ...
    assert (
        body["control_owner"],
        body["control_owner_since_ms"],
        body["control_owner_app_id"],
    ) == ("mba", 1, "APP_MBA")
    assert body["thread_control"] == {
        "last_action": "release",
        "last_ok": True,
    }  # el operador ve el último release/error
    assert body["history"] == history[-mba_api.CONTROL_HISTORY_LIMIT :]


@pytest.mark.parametrize(
    "bad_key",
    ["573001234567", "wa_abc", "wa_..", "wa_5730012345671234567", "wa_573001234567%0A"],
)
def test_session_control_rejects_keys_that_are_not_a_phone_session(
    bad_key: str,
) -> None:
...
class _ReleaseStub:
    ...
    async def execute(
        ...
        *,
        ...
        agent_event_emitted=False,
        metadata=None,
    ):
        from src.plugins.mba.use_cases.release_thread import ReleaseOutcome

        self.calls.append(
            (session_key, trigger, order_registered, agent_event_emitted, metadata)
        )
...
def test_release_endpoint_delegates_to_the_use_case_with_the_trigger_and_facts() -> (
    None
):
    ...
    r = c.post(
        "/api/mba/sessions/wa_573001234567/control/release",
        json={
            "trigger": "handoff_resolved",
            "metadata": "caso cerrado",
    ...
    assert (
        c.post("/api/mba/sessions/wa_573001234567/control/release").status_code == 200
    )
...
def test_release_endpoint_maps_guard_reasons_to_status_codes(
    reason: str, status: int
) -> None:
    r = _release_client(_ReleaseStub(reason)).post(
        "/api/mba/sessions/wa_573001234567/control/release"
    )
...
class _AgentEventStub:
    ...
    async def execute(
        ...
        *,
        ...
        self.calls.append(
            (session_key, event_type, order_id, message, payload, source, episode_id)
        )
        return AgentEventOutcome(
            session_key=session_key,
            emitted=self.emitted,
            reason=self.reason,
            agent_event_id="AE_1" if self.emitted else None,
            at_ms=1,
        )
...
def test_agent_event_endpoint_delegates_to_the_use_case() -> None:
    ...
    r = c.post(
        "/api/mba/sessions/wa_573001234567/agent-events",
        json={
            "type": "order_shipped",
            "order_id": "order_1",
    ...
    assert (
        r.status_code == 200
        and stub.calls[-1][1] == "episode_closed"
        and stub.calls[-1][6] == "ep_002"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("unavailable", 200),
        ("ambiguous", 200),
        ("entity_id_missing", 200),
    ],
)
def test_agent_event_endpoint_maps_guard_reasons_to_status_codes(
    reason: str, status: int
) -> None:
    r = _agent_event_client(_AgentEventStub(reason)).post(
        "/api/mba/sessions/wa_573001234567/agent-events",
        json={"type": "order_shipped", "message": "m"},
    )
    assert r.status_code == status
    if status == 200:
...
def test_agent_event_endpoint_validates_key_type_and_message() -> None:
    ...
    assert (
        c.post("/api/mba/sessions/573001234567/agent-events", json=ok).status_code
        == 422
    )
    assert (
        c.post(
            "/api/mba/sessions/wa_573001234567/agent-events",
            json={**ok, "type": "whatever"},
        ).status_code
        == 422
    )
    assert (
        c.post(
            "/api/mba/sessions/wa_573001234567/agent-events", json={**ok, "message": ""}
        ).status_code
        == 422
    )
    assert (
        c.post(
            "/api/mba/sessions/wa_573001234567/agent-events",
            json={**ok, "message": "x" * 2001},
        ).status_code
        == 422
    )
    assert (
        c.post(
            "/api/mba/sessions/wa_573001234567/agent-events", json={"message": "m"}
        ).status_code
        == 422
    )
    big = {"blob": "x" * (8 * 1024)}
...
class _SyncStub:
    def __init__(self, *, plan=None, outcome=None, plan_error=None) -> None:
        ...
        self._plan = (
            plan if plan is not None else SyncPlan("sales", "PHONE_777", (), (), "fp-1")
        )
    ...
    async def apply(self, agent_id, *, fingerprint=None):
        ...
        return self._outcome or SyncOutcome(
            agent_id, True, "applied", status="ok", plan={"fingerprint": "fp-1"}
        )


def _sync_client(
    stub: _SyncStub, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> TestClient:
    from src.plugins.mba import api as mba_api
...
def test_sync_state_is_served_from_the_vault_and_is_empty_before_the_first_run(
    ...
    SyncStateStore(tmp_path).write(
        "sales", {"ids": {"skills": {"persona": "s-1"}}, "last_apply": {"status": "ok"}}
    )
    assert client.get("/api/mba/agents/sales/sync").json()["state"]["last_apply"] == {
        "status": "ok"
    }
...
def test_sync_plan_is_read_only_and_reports_a_remote_outage_as_503(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    down = _SyncStub(
        plan_error=MbaAdminError("unavailable", status=503, detail="down", attempts=3)
    )
    res = _sync_client(down, tmp_path, monkeypatch).get(
        "/api/mba/agents/sales/sync/plan"
    )
    assert res.status_code == 503 and res.json()["detail"] == {
    ...
    unconfigured = _SyncStub(
        plan_error=MbaAdminError(
            "not_configured", detail="META_MBA_TOKEN no configurado"
        )
    )
    res = _sync_client(unconfigured, tmp_path, monkeypatch).get(
        "/api/mba/agents/sales/sync/plan"
    )
    assert res.status_code == 503 and res.json()["detail"]["kind"] == "not_configured"


def test_sync_apply_passes_the_confirmed_fingerprint_and_maps_guards_to_status_codes(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from src.plugins.mba.use_cases.sync_agent import SyncOutcome
    ...
    assert (
        res.status_code == 200
        and res.json()["applied"] is True
        and res.json()["reason"] == "applied"
    )
    ...
    assert (
        client.post("/api/mba/agents/sales/sync", json={"fingerprint": ""}).status_code
        == 422
    )
    assert stub.calls == [("apply", "sales", "fp-1")]
    ...
    for reason, status in (
        ...
        out = SyncOutcome(
            ...
            False,
            reason,
            error={"kind": "unavailable", "detail": "down", "status": 503}
            if reason == "remote_unavailable"
            else None,
        )
        res = _sync_client(_SyncStub(outcome=out), tmp_path, monkeypatch).post(
            "/api/mba/agents/sales/sync", json={"fingerprint": "fp-1"}
        )
    ...
    for reason in ("blocked", "plan_changed", "nothing_to_do"):
        out = SyncOutcome(
            ...
            blocked=("placeholder:<FLOW_ID>",) if reason == "blocked" else (),
        )
        res = _sync_client(_SyncStub(outcome=out), tmp_path, monkeypatch).post(
            "/api/mba/agents/sales/sync", json={"fingerprint": "fp-1"}
        )
        assert (
            res.status_code == 200
            and res.json()["reason"] == reason
            and res.json()["applied"] is False
        )
    assert (
        client.post(
            "/api/mba/agents/sales/sync", json={"fingerprint": "x" * 200}
        ).status_code
        == 422
    )


def test_the_real_sync_agent_is_wired_with_the_vault_store_the_flag_and_the_connector_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from src.plugins.mba import api as mba_api
...
class _RolloutStub:
    def __init__(self, *, status=None, outcome=None, status_error=None) -> None:
        self._status, self._outcome, self._status_error, self.calls = (
            status,
            outcome,
            status_error,
            [],
    ...
    async def status(self, agent_id):
        ...
        return self._status or RolloutStatus(
            ...
            checks=[{"code": "flag_enabled", "ok": True, "detail": ""}],
            can_enable=True,
...
def test_rollout_status_is_served_and_a_remote_outage_is_503() -> None:
    ...
    body = res.json()
    assert (
        body["rollout_enabled"] is False
        and body["can_enable"] is True
        and body["allowlist"][0]["phone"] == "+573001234567"
    )
    assert _rollout_client(stub).get("/api/mba/agents/nope/rollout").status_code == 404
    down = _RolloutStub(
        status_error=MbaAdminError("unavailable", status=503, detail="down", attempts=3)
    )
    res = _rollout_client(down).get("/api/mba/agents/sales/rollout")
    assert (
        res.status_code == 503 and res.json()["detail"]["error"] == "remote_unavailable"
    )
...
def test_rollout_writes_delegate_with_the_confirmation_and_validate_input() -> None:
    ...
    assert (
        c.post(
            "/api/mba/agents/sales/rollout/allowlist", json={"phone": "+573009876543"}
        ).status_code
        == 200
    )
    ...
    assert (
        c.put(
            "/api/mba/agents/sales/rollout/audience",
            json={"ai_audience": "EVERYONE", "confirm": True},
        ).status_code
        == 200
    )
    assert (
        c.put(
            "/api/mba/agents/sales/rollout/enabled",
    ...
    assert (
        c.put(
            "/api/mba/agents/sales/rollout/enabled", json={"enabled": False}
        ).status_code
        == 200
    )
    assert stub.calls == [
    ...
    assert (
        c.put(
            "/api/mba/agents/sales/rollout/audience", json={"ai_audience": "FRIENDS"}
        ).status_code
        == 422
    )
    assert c.delete("/api/mba/agents/sales/rollout/allowlist/..%2Fx").status_code in (
        404,
        422,
    )  # starlette normaliza el path
    assert c.delete("/api/mba/agents/sales/rollout/allowlist/e%201").status_code == 422
    assert (
        c.put(
            "/api/mba/agents/nope/rollout/enabled", json={"enabled": False}
        ).status_code
        == 404
    )
...
def test_rollout_guards_map_to_status_codes_and_policy_refusals_are_200(
    reason: str, status: int
) -> None:
    from src.plugins.mba.use_cases.rollout_control import RolloutOutcome

    out = RolloutOutcome(
        "sales", False, reason, error={"kind": reason, "detail": "x", "status": None}
    )
    res = _rollout_client(_RolloutStub(outcome=out)).put(
        "/api/mba/agents/sales/rollout/enabled", json={"enabled": True, "confirm": True}
    )
    ...
    for policy in (
        ...
        "customer_not_in_hubara_allowlist",
        "already_listed",
        "rejected",
    ):
        out = RolloutOutcome(
            ...
            False,
            policy,
            blocked=("sync_ok",) if policy == "not_ready" else (),
        )
        res = _rollout_client(_RolloutStub(outcome=out)).put(
            "/api/mba/agents/sales/rollout/enabled",
            json={"enabled": True, "confirm": True},
        ...
        assert (
            res.status_code == 200
            and res.json()["applied"] is False
            and res.json()["reason"] == policy
        )


def test_the_real_rollout_control_is_wired_with_hubaras_closed_list_and_the_everyone_knob(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from src.plugins.mba import api as mba_api
    ...
    assert (
        uc._hubara_allowed("+573001234567") is True
        and uc._hubara_allowed("+573000000000") is False
    )
...
class _AdminTestStub:
    ...
    async def agent_test(self, entity_id, user_msg, *, conversation_id=None):
        ...
        return self._reply or {
            "message_id": "m1",
            "agent_response": "Hola, soy el asesor.",
            "conversation_id": "conv-1",
            "timestamp": 1,
        }


def _agent_test_client(
    stub: _AdminTestStub,
    monkeypatch: pytest.MonkeyPatch,
    entity_id: str | None = "PHONE_777",
) -> TestClient:
    from src.plugins.mba import api as mba_api
...
def test_agent_test_sends_the_message_to_meta_and_threads_the_conversation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    stub = _AdminTestStub()
    ...
    assert (
        body["ok"] is True
        and body["reply"]["agent_response"] == "Hola, soy el asesor."
        and body["reply"]["conversation_id"] == "conv-1"
    )
    res = c.post(
        "/api/mba/agents/sales/test",
        json={"message": "¿y envíos?", "conversation_id": "conv-1"},
    )
    assert res.status_code == 200
    assert stub.calls == [
        ("PHONE_777", "hola", None),
        ("PHONE_777", "¿y envíos?", "conv-1"),
    ]
    ...
    rich = _AdminTestStub(
        reply={
            ...
            "conversation_id": "conv-1",
            "timestamp": 2,
            "no_response_reason": "handoff",
            "handoff_reason": "customer_asked_for_human",
            "quick_replies": ["Sí", "No"],
            "product_variant_ids": ["v1"],
        }
    )
    body = (
        _agent_test_client(rich, monkeypatch)
        .post("/api/mba/agents/sales/test", json={"message": "quiero un humano"})
        .json()
    )
    assert body["reply"]["handoff_reason"] == "customer_asked_for_human" and body[
        "reply"
    ]["quick_replies"] == ["Sí", "No"]


def test_agent_test_validates_input_and_maps_guards(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from src.plugins.mba.adapters.meta_admin import MbaAdminError
    ...
    assert (
        c.post("/api/mba/agents/sales/test", json={"message": "   \n "}).status_code
        == 422
    )  # solo espacios
    assert (
        c.post("/api/mba/agents/sales/test", json={"message": "x" * 4097}).status_code
        == 422
    )
    assert (
        c.post(
            "/api/mba/agents/sales/test",
            json={"message": "hola", "conversation_id": "x" * 200},
        ).status_code
        == 422
    )
    assert (
        c.post("/api/mba/agents/nope/test", json={"message": "hola"}).status_code == 404
    )
    assert stub.calls == []
    # sin entity_id (número no onboardeado): 409 antes de tocar Meta
    res = _agent_test_client(stub, monkeypatch, entity_id=None).post(
        "/api/mba/agents/sales/test", json={"message": "hola"}
    )
    assert (
        res.status_code == 409
        and res.json()["detail"]["error"] == "entity_id_missing"
        and stub.calls == []
    )
    ...
    for kind, status in (
        ...
        down = _AdminTestStub(
            error=MbaAdminError(
                kind, status=503 if kind == "unavailable" else None, detail="down"
            )
        )
        res = _agent_test_client(down, monkeypatch).post(
            "/api/mba/agents/sales/test", json={"message": "hola"}
        )
        assert res.status_code == status and res.json()["detail"]["kind"] == kind
    rejected = _AdminTestStub(
        error=MbaAdminError("rejected", status=400, detail="agent not onboarded")
    )
    res = _agent_test_client(rejected, monkeypatch).post(
        "/api/mba/agents/sales/test", json={"message": "hola"}
    )
    assert res.status_code == 200 and res.json() == {
        "ok": False,
        "reply": None,
        "error": {"kind": "rejected", "status": 400, "detail": "agent not onboarded"},
    }
