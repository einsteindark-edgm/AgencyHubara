...
def test_enabling_is_blocked_by_any_failed_guard() -> None:
    ...
    assert can_enable(
        _facts(allowlist=(("e1", "+573001234567"), ("e2", "+573000000000")))
    ) == ("allowlist_within_hubara",)
    bad = next(
        c
        for c in readiness(_facts(allowlist=(("e2", "+573000000000"),)))
        if c.code == "allowlist_within_hubara"
    )
...
def test_only_hubara_allowed_e164_phones_can_join_metas_allowlist() -> None:
    ...
    assert (
        can_add_phone("+573000000000", _facts()) == "customer_not_in_hubara_allowlist"
    )
...
def test_everyone_needs_the_policy_knob_and_an_explicit_confirmation() -> None:
    assert (
        can_set_audience("EVERYONE", confirm=True, facts=_facts())
        == "everyone_not_allowed"
    )
    assert (
        can_set_audience("EVERYONE", confirm=False, facts=_facts(everyone_knob=True))
        == "confirmation_required"
    )
    assert (
        can_set_audience("EVERYONE", confirm=True, facts=_facts(everyone_knob=True))
        is None
    )
    assert can_set_audience("ALLOWLISTED_ONLY", confirm=False, facts=_facts()) is None
    assert (
        can_set_audience(
            "ALLOWLISTED_ONLY", confirm=False, facts=_facts(flag_enabled=False)
        )
        is None
    )  # volver a cerrado siempre
    assert (
        can_set_audience("FRIENDS", confirm=True, facts=_facts(everyone_knob=True))
        == "invalid_audience"
    )
...
def test_everyone_cannot_be_opened_while_mba_is_on() -> None:
    ...
    assert (
        can_set_audience(
            "EVERYONE",
            confirm=True,
            facts=_facts(everyone_knob=True, rollout_enabled=True),
        )
        == "disable_first"
    )
    assert (
        can_set_audience(
            "ALLOWLISTED_ONLY", confirm=False, facts=_facts(rollout_enabled=True)
        )
        is None
    )
