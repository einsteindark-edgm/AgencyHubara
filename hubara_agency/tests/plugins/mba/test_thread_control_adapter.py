...
def test_url_and_version_follow_metas_openapi() -> None:
    ...
    assert THREAD_CONTROL_API_VERSION == "1.0.0"  # el OpenAPI de thread_control solo enumera 1.0.0
...
async def test_release_posts_the_documented_body_and_headers() -> None:
    route = respx.post(URL).mock(return_value=httpx.Response(200, json={"messaging_product": "whatsapp"}))
    res = await _client().release(phone_number_id=PHONE, to=CUSTOMER, metadata="hubara:handoff_resolved")
    ...
    assert json.loads(req.content) == {
        "messaging_product": "whatsapp", "action": "release", "to": CUSTOMER, "metadata": "hubara:handoff_resolved",
    }
...
async def test_take_omits_metadata_when_absent() -> None:
    ...
    assert json.loads(route.calls.last.request.content) == {"messaging_product": "whatsapp", "action": "take", "to": CUSTOMER}
...
async def test_a_4xx_is_a_rejection_with_metas_message_and_is_not_retried() -> None:
    body = {"error": {"message": "(#100) You must hold thread control to release it", "code": 100, "fbtrace_id": "X"}}
...
async def test_the_fake_records_calls_and_can_fail_on_demand() -> None:
    ...
    fake.fail_with = ThreadControlError("unavailable", status=500, detail="x", attempts=3)
