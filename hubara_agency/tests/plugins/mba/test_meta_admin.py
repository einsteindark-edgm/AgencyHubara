...
def test_urls_and_version_follow_metas_openapi() -> None:
    ...
    assert admin_url(MBA_API_BASE_URL, ENTITY, "agent_eligibility") == f"{BASE}/agent_eligibility"
    assert admin_url(MBA_API_BASE_URL, ENTITY, "agent_config/skills", "sk1") == f"{BASE}/agent_config/skills/sk1"
    assert (
...
def test_the_client_paths_match_the_preview_catalogue() -> None:
    ...
    for ep in ENDPOINTS:
        expected = ep.path.replace("{entity_id}", ENTITY).replace("{connector_id}", "CID")
        got = admin_url(MBA_API_BASE_URL, ENTITY, RESOURCE_BY_SECTION[ep.section].format(connector_id="CID"))
...
async def test_eligibility_is_a_get_with_the_bearer_and_version_headers() -> None:
    route = respx.get(f"{BASE}/agent_eligibility").mock(return_value=httpx.Response(200, json={"is_eligible": True}))
    assert await _client().eligibility(ENTITY) is True
    req = route.calls.last.request
    assert req.headers["Authorization"] == "Bearer tok"
    assert req.headers["X-API-Version"] == ADMIN_API_VERSION
    respx.get(f"{BASE}/agent_eligibility").mock(return_value=httpx.Response(200, json={"is_eligible": False}))
...
async def test_onboarding_posts_an_empty_body_unless_a_catalog_is_linked() -> None:
    route = respx.post(f"{BASE}/agent_onboarding").mock(return_value=httpx.Response(201, json={"agent_id": "ag_1"}))
...
async def test_settings_get_returns_the_list_and_put_sends_the_body_verbatim() -> None:
    settings = {"agent_id": "ag_1", "channel": "whatsapp", "rollout": {"enabled": False}, "ai_audience": "ALLOWLISTED_ONLY"}
    get = respx.get(f"{BASE}/agent_config/settings").mock(return_value=httpx.Response(200, json=[settings]))
    ...
    put = respx.put(f"{BASE}/agent_config/settings").mock(return_value=httpx.Response(200, json=settings))
    body = {"rollout": {"enabled": False}, "ai_audience": "ALLOWLISTED_ONLY", "never_say_phrases": ["gratis"]}
...
async def test_business_info_get_put_and_delete() -> None:
    info = {"business_description": "Velas", "contact_info": {"email": "hola@hubara.co"}}
    respx.get(f"{BASE}/agent_config/business_info").mock(return_value=httpx.Response(200, json=info))
    put = respx.put(f"{BASE}/agent_config/business_info").mock(return_value=httpx.Response(200, json=info))
    delete = respx.delete(f"{BASE}/agent_config/business_info").mock(return_value=httpx.Response(204))
    c = _client()
...
async def test_faqs_are_a_crud_by_id() -> None:
    ...
    lst = respx.get(f"{BASE}/agent_config/faq").mock(return_value=httpx.Response(200, json=[faq]))
    create = respx.post(f"{BASE}/agent_config/faq").mock(return_value=httpx.Response(201, json=faq))
    update = respx.put(f"{BASE}/agent_config/faq/f1").mock(return_value=httpx.Response(200, json=faq))
    delete = respx.delete(f"{BASE}/agent_config/faq/f1").mock(return_value=httpx.Response(204))
    c = _client()
    ...
    assert await c.update_faq(ENTITY, "f1", {"question": "¿Envían?", "answer": "Sí, a todo el país"}) == faq
...
async def test_skills_are_a_crud_by_id_with_the_agent_id_query() -> None:
    skill = {"id": "s1", "title": "saludo", "description": "d", "skill": "texto", "channel": "whatsapp", "status": "active"}
    lst = respx.get(f"{BASE}/agent_config/skills").mock(return_value=httpx.Response(200, json=[skill]))
    create = respx.post(f"{BASE}/agent_config/skills").mock(return_value=httpx.Response(201, json=skill))
    update = respx.put(f"{BASE}/agent_config/skills/s1").mock(return_value=httpx.Response(200, json=skill))
    delete = respx.delete(f"{BASE}/agent_config/skills/s1").mock(return_value=httpx.Response(204))
    c = _client()
    ...
    assert _last_json(create) == body and create.calls.last.request.url.params["agent_id"] == "ag_1"
...
async def test_connectors_crud_and_api_key_upsert() -> None:
    con = {"id": "c1", "name": "hubara", "base_url": "https://x", "auth_type": "API_KEY", "connection_status": {"status": "ACTIVE"}}
    respx.get(f"{BASE}/agent_connectors").mock(return_value=httpx.Response(200, json=[con]))
    create = respx.post(f"{BASE}/agent_connectors").mock(return_value=httpx.Response(201, json=con))
    update = respx.put(f"{BASE}/agent_connectors/c1").mock(return_value=httpx.Response(200, json=con))
    key = respx.post(f"{BASE}/agent_connectors/c1/upsertApiKey").mock(return_value=httpx.Response(200, json=con))
    delete = respx.delete(f"{BASE}/agent_connectors/c1").mock(return_value=httpx.Response(204))
    c = _client()
    assert await c.list_connectors(ENTITY) == [con]
    body = {"name": "hubara", "description": "d", "base_url": "https://x", "auth_type": "API_KEY"}
...
async def test_connector_tools_live_under_their_connector() -> None:
    tool = {"id": "t1", "name": "check_order_status", "description": "d", "request_definition": {"method": "GET", "path": "/x"}, "user_auth_required": False}
    respx.get(f"{BASE}/agent_connectors/c1/tools").mock(return_value=httpx.Response(200, json=[tool]))
    create = respx.post(f"{BASE}/agent_connectors/c1/tools").mock(return_value=httpx.Response(201, json=tool))
    update = respx.put(f"{BASE}/agent_connectors/c1/tools/t1").mock(return_value=httpx.Response(200, json=tool))
    delete = respx.delete(f"{BASE}/agent_connectors/c1/tools/t1").mock(return_value=httpx.Response(204))
    c = _client()
...
async def test_ui_skills_list_follows_the_cursor_until_the_last_page() -> None:
    a = {"id": "u1", "title": "catalogo", "component_type": "carousel_url", "status": "enabled", "instruction": "i"}
    b = {"id": "u2", "title": "flow", "component_type": "flow", "status": "enabled", "instruction": "i"}
    route = respx.get(f"{BASE}/agent-ui-skills").mock(side_effect=[
        httpx.Response(200, json={"data": [a], "paging": {"cursors": {"before": "x", "after": "CUR"}, "next": "https://api.facebook.com/next"}}),
        httpx.Response(200, json={"data": [b], "paging": {"cursors": {"before": "CUR", "after": "END"}}}),
    ])
...
async def test_ui_skills_create_update_and_delete() -> None:
    ui = {"id": "u1", "title": "catalogo", "component_type": "carousel_url", "status": "enabled", "instruction": "i"}
    create = respx.post(f"{BASE}/agent-ui-skills").mock(return_value=httpx.Response(201, json=ui))
    update = respx.put(f"{BASE}/agent-ui-skills/u1").mock(return_value=httpx.Response(200, json=ui))
    delete = respx.delete(f"{BASE}/agent-ui-skills/u1").mock(return_value=httpx.Response(204))
    c = _client()
...
async def test_allowlist_list_add_and_remove_by_entry_id() -> None:
    ...
    respx.get(f"{BASE}/agent_config/allowlist").mock(return_value=httpx.Response(200, json=[entry]))
    add = respx.post(f"{BASE}/agent_config/allowlist").mock(return_value=httpx.Response(201, json=entry))
    delete = respx.delete(f"{BASE}/agent_config/allowlist/e1").mock(return_value=httpx.Response(204))
    c = _client()
...
async def test_agent_test_sends_the_message_and_threads_the_conversation() -> None:
    ...
    route = respx.post(f"{BASE}/agent_test").mock(return_value=httpx.Response(200, json=reply))
    c = _client()
...
async def test_a_4xx_is_rejected_with_metas_standard_error_detail() -> None:
    respx.post(f"{BASE}/agent_config/skills").mock(
        return_value=httpx.Response(400, json={"title": "Bad Request", "detail": "skill exceeds 20000 chars", "status": 400})
    )
    with pytest.raises(MbaAdminError) as exc:
        await _client(token="SECRET").create_skill(ENTITY, {"title": "x", "description": "d", "skill": "s"})
...
async def test_intermittent_5xx_and_429_are_retried_with_backoff() -> None:
    sleeper = _Sleeper()
    route = respx.get(f"{BASE}/agent_config/skills").mock(side_effect=[
...
async def test_persistent_5xx_is_unavailable_after_the_attempts() -> None:
    route = respx.put(f"{BASE}/agent_config/settings").mock(
        return_value=httpx.Response(503, json={"title": "Unavailable", "detail": "down"})
    )
    with pytest.raises(MbaAdminError) as exc:
...
async def test_transport_errors_are_retried_on_idempotent_calls() -> None:
    ...
    route = respx.put(f"{BASE}/agent_config/business_info").mock(side_effect=[
        httpx.ReadTimeout("slow"),
        httpx.Response(200, json={"business_description": "Velas"}),
    ])
    assert await _client().put_business_info(ENTITY, {"business_description": "Velas"}) == {"business_description": "Velas"}
    assert route.call_count == 2
...
async def test_a_transport_error_on_a_create_is_ambiguous_and_not_retried() -> None:
    ...
    route = respx.post(f"{BASE}/agent_config/faq").mock(side_effect=httpx.ReadTimeout("slow"))
    with pytest.raises(MbaAdminError) as exc:
