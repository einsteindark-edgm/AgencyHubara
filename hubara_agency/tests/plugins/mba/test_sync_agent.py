...
def test_the_state_store_writes_one_json_per_agent_under_the_vault(
    tmp_path: Path,
) -> None:
    store = SyncStateStore(tmp_path)
    ...
    assert (
        path.is_file()
        and json.loads(path.read_text())["ids"]["skills"]["persona"] == "s-1"
    )
...
async def test_plan_reads_meta_without_writing_anything(tmp_path: Path) -> None:
    ...
    assert all(
        ...
        for op, *_ in fake.calls
    )
    assert store.read("sales") == {}
    assert await uc.plan("nope") is None


async def test_plan_reports_a_remote_outage_instead_of_an_empty_diff(
    tmp_path: Path,
) -> None:
...
async def test_first_apply_creates_everything_and_the_second_run_changes_nothing(
    tmp_path: Path,
) -> None:
    uc, fake, store = _agent(tmp_path)
    ...
    assert [r["action"] for r in out.results] == [
        "update",
        "create",
        "create",
        "create",
    ...
    assert [t["name"] for t in fake.tools[(ENTITY, "connector-1")]] == [
        "search_products",
        "register_order",
    ]
    assert (
        fake.connectors[ENTITY][0]["auth_config"]["api_key"]["headers"][0]["value"]
        == API_KEY
    )
    # …y rollout / ai_audience NO se tocaron (PUT parcial sin esos campos)
    assert (
        fake.settings[ENTITY]["rollout"] == {"enabled": True}
        and fake.settings[ENTITY]["ai_audience"] == "EVERYONE"
    )
    ...
    assert state["ids"]["skills"] == {"persona": "skill-1"} and state["ids"][
        "connector"
    ] == {"hubara-commerce": "connector-1"}
    assert state["ids"]["connector_tools"] == {
        "search_products": "tool-1",
        "register_order": "tool-2",
    }
    assert state["sent"]["settings"]["never_say_phrases"]
    assert (
        state["last_apply"]["status"] == "ok"
        and state["last_apply"]["at_ms"] == 1_700_000_000_000
    )
...
async def test_apply_updates_by_key_and_deletes_only_what_we_created(
    tmp_path: Path,
) -> None:
    uc, fake, store = _agent(tmp_path)
    await uc.apply("sales")
    # alguien creó una skill a mano y una FAQ nuestra ya no está en el workspace
    await fake.create_skill(
        ENTITY, {"title": "hecha-a-mano", "description": "", "skill": "x"}
    )
    fake.calls.clear()
    changed = _cfg(
    ...
    assert [s["title"] for s in fake.skills[ENTITY]] == [
        "persona",
        "hecha-a-mano",
    ]  # lo ajeno sigue
...
async def test_apply_is_refused_when_the_plan_is_blocked_disabled_or_stale(
    tmp_path: Path,
) -> None:
    with_placeholder = _cfg(
        _YAML.replace(
            'instruction: "Envía el formulario."', 'instruction: "Flow <FLOW_ID>."'
        )
    )
    ...
    assert (
        ...
        and out.reason == "blocked"
        and "placeholder:<FLOW_ID>" in out.blocked
    )
    assert not [
        c for c in fake.calls if c[0].startswith(("create", "update", "put", "delete"))
    ]
    ...
    assert (
        out.reason == "plan_changed" and out.plan["fingerprint"] != "stale-fingerprint"
    )
    assert not [
        c for c in fake.calls if c[0].startswith(("create", "update", "put", "delete"))
    ]
...
async def test_a_rejected_item_is_recorded_and_the_rest_continues_but_an_outage_aborts(
    tmp_path: Path,
) -> None:
    class _Admin(FakeMbaAdmin):
        async def create_skill(self, entity_id, body, *, agent_id=None):
            raise MbaAdminError(
                "rejected", status=400, detail="skill blocked by policy"
            )
    ...
    assert [
        (r["section"], r["error"]["kind"], r["error"]["detail"]) for r in failed
    ] == [("skills", "rejected", "skill blocked by policy")]
    ...
    assert all(
        ...
        if r["section"] in ("connector_tools", "ui_skills", "settings")
    )
    # lo creado antes del corte quedó registrado: el próximo run no lo duplica
    state = store.read("sales")
    assert (
        state["ids"]["skills"] == {"persona": "skill-1"}
        and "connector" not in state["ids"]
    )
...
async def test_a_remote_outage_before_applying_writes_nothing(tmp_path: Path) -> None:
    ...
    assert (
        out.applied is False
        and out.reason == "remote_unavailable"
        and out.error == {"kind": "unavailable", "detail": "down", "status": 503}
    )
    assert store.read("sales") == {}
...
async def test_updating_a_foreign_item_does_not_register_it_for_deletion(
    tmp_path: Path,
) -> None:
    fake = FakeMbaAdmin()
    await fake.create_faq(
        ENTITY, {"question": "¿Envían a Cali?", "answer": "Depende."}
    )  # ajena
    ...
    out = await uc2.apply("sales")
    assert out.reason == "nothing_to_do" and [
        f["question"] for f in fake.faqs[ENTITY]
    ] == ["¿Envían a Cali?", "¿Cuánto demora?"]


async def test_a_replace_whose_create_fails_reports_the_old_skill_as_gone(
    tmp_path: Path,
) -> None:
    uc, fake, store = _agent(tmp_path)
    await uc.apply("sales")
    ...
    assert (
        ...
        and row["error"]["detail"].startswith("borrada, no recreada")
    )
    assert fake2.ui_skills[ENTITY] == []  # la vieja SÍ desapareció en Meta
    assert (
        "request-shipping-details" not in store.read("sales")["ids"]["ui_skills"]
    )  # y el vault lo sabe


async def test_the_vault_never_stores_a_hash_of_the_real_api_key(
    tmp_path: Path,
) -> None:
    import hashlib
