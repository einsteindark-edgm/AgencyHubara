...
@pytest.mark.asyncio
async def test_status_lists_registered_orders_with_live_stage_and_pay_status(
    tmp_path: Path,
) -> None:
    store = _vault(
        tmp_path,
    ...
    query = _Query(
        {
            ...
            "order_2": SimpleNamespace(
                status="new", pay_status="pending", display_id="#1248", total_cop=35000
            ),
        }
    )
    ...
    assert (
        first["status"] == "en camino"
        and first["payment_confirmed"] is True
        and first["display_id"] == "#1247"
    )
    assert (
        second["status"] == "recibido"
        and second["payment_confirmed"] is False
        and second["pay_status"] == "pending"
    )
    assert "note" not in out


@pytest.mark.asyncio
async def test_status_is_scoped_to_the_customer_and_degrades_when_live_lookup_fails(
    tmp_path: Path,
) -> None:
    ...
    degraded = await check_order_status(
        store, _Query({}, boom=True), session_key=_SESSION
    )
...
async def test_checkout_verification_passes_items_and_reports_discrepancies() -> None:
    ...
    out = await verify_order_for_checkout(
        v,
        items=[
            {
                "handle": "vela-lavanda",
    ...
    ok = await verify_order_for_checkout(
        _Verifier(
            CheckoutVerification(verified=True, catalog_available=True, items=[])
        ),
...
async def test_checkout_without_live_source_tells_the_agent_to_escalate() -> None:
    down = await verify_order_for_checkout(
        _Verifier(
            CheckoutVerification(
                verified=False, catalog_available=False, error_detail="timeout"
            )
        ),
        items=[{"handle": "x", "quantity": 1}],
    )
    assert (
        down["error"] == "catalog_unavailable"
        and "CHECKOUT_VERIFY_FAILED" in down["message"]
    )
    assert (
        await verify_order_for_checkout(None, items=[{"handle": "x", "quantity": 1}])
    )["error"] == "catalog_unavailable"


@pytest.mark.asyncio
async def test_live_lookups_run_in_parallel_under_one_budget_and_truncate_to_max(
    ...
    store = _vault(
        tmp_path, _SESSION, {"episodes": [{"order_id": f"order_{i}"} for i in range(8)]}
    )
    ...
    class _Slow:
        async def get(self, order_id: str):
            ...
            return SimpleNamespace(
                summary=SimpleNamespace(status="new", pay_status="pending")
            )
...
@pytest.mark.asyncio
async def test_checkout_rejects_non_positive_quantities_and_hides_vendor_errors() -> (
    None
):
    ...
    down = await verify_order_for_checkout(
        _Verifier(
            CheckoutVerification(
                ...
                error_detail="ConnectError https://medusa.internal:9000/admin",
            )
        ),
        items=[{"handle": "x", "quantity": 1}],
    )
    assert (
        "medusa.internal" not in str(down) and down["detail"] == "checkout_unavailable"
    )
