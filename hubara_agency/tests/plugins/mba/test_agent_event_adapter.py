...
def test_url_and_version_follow_metas_openapi() -> None:
    ...
    )  # agent_event usa 2.0.0 (thread_control es el único en 1.0.0)
    assert isinstance(MetaAgentEvent(token=lambda: "x"), AgentEventPort)
    assert isinstance(FakeAgentEvent(), AgentEventPort)


@respx.mock
async def test_emit_posts_the_documented_body_with_the_payload_as_a_json_string() -> (
    None
):
    route = respx.post(URL).mock(
        return_value=httpx.Response(
            200, json={"status": "accepted", "agent_event_id": "AE_1"}
        )
    )
    res = await _client().emit(
        ...
        description="Cuéntale al cliente que su pedido va en camino.",
    ...
    assert (
        ...
        == "Cuéntale al cliente que su pedido va en camino."
    )
    assert json.loads(body["event"]["payload"]) == {
        "order_id": "order_1",
        "stage": "shipping",
    }
    assert set(body) == {"to", "event"} and set(body["event"]) == {
        "type",
        "description",
        "payload",
    }
...
@respx.mock
async def test_emit_without_payload_omits_the_key_and_tolerates_a_bodyless_ack() -> (
    None
):
    route = respx.post(URL).mock(
    ...
    res = await _client().emit(
        entity_id=ENTITY, to=TO, event_type="payment_received", description="d"
    )
    assert res == AgentEventResult(agent_event_id=None, status="accepted")
    assert set(json.loads(route.calls.last.request.content)["event"]) == {
        "type",
        "description",
    }
...
async def test_without_a_token_nothing_is_called() -> None:
    ...
    for token in ("", "PLACEHOLDER_set_out_of_band"):
        with pytest.raises(AgentEventError) as exc:
            await _client(token=token).emit(
                entity_id=ENTITY, to=TO, event_type="order_shipped", description="d"
            )
...
async def test_a_4xx_is_a_rejection_with_metas_message_and_is_not_retried() -> None:
    route = respx.post(URL).mock(
        return_value=httpx.Response(400, json={"error": {"message": "Unknown entity"}})
    )
    with pytest.raises(AgentEventError) as exc:
        await _client().emit(
            entity_id=ENTITY, to=TO, event_type="order_shipped", description="d"
        )
    assert (exc.value.kind, exc.value.status, exc.value.detail, exc.value.attempts) == (
        "rejected",
        400,
        "Unknown entity",
        1,
...
async def test_a_5xx_is_retried_with_backoff_then_reported_as_unavailable() -> None:
    ...
    with pytest.raises(AgentEventError) as exc:
        await _client(sleeper=sleeper).emit(
            entity_id=ENTITY, to=TO, event_type="order_shipped", description="d"
        )
    assert (
        exc.value.kind == "unavailable"
        and route.call_count == 3
        and sleeper.delays == [0.5, 1.0]
    )


@respx.mock
async def test_a_timeout_is_ambiguous_and_is_not_retried_because_the_event_is_not_idempotent() -> (
    None
):
