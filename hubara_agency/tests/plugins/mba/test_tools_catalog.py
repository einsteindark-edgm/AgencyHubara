...
from src.plugins.mba.tools.catalog import (
...
_MANIFEST = CatalogManifestDTO(
    version="v1", fetched_at="2026-09-07T00:00:00Z", product_count=2
)
...
_VELA = CatalogProductDTO(
    ...
    thumbnail="https://cdn/vela.webp",
    variants=[
        CatalogVariantDTO(
            id="var_1",
            title="Unico",
            prices=[
                CatalogPriceDTO(amount="9.00", currency_code="usd"),
...
_ZODIAC = CatalogProductDTO(
    ...
    images=[
        CatalogImageDTO(url="https://cdn/z/00-portada-duo.webp", rank=0),
        CatalogImageDTO(url="https://cdn/z/leo-1.webp", rank=1),
        CatalogImageDTO(url="https://cdn/z/aries-1.webp", rank=2),
    ],
    tags=[],
    categories=["zodiacal"],
    metadata={"colores": "Leo: naranja; Aries: rojo"},
...
class _FakeCatalog:
    ...
    async def search(
        ...
        if category is not None:
            matched = (
                CatalogCategoryDTO(
                    slug="velas-aromaticas", label="Velas Aromáticas", product_count=1
                )
                if "arom" in category
                else None
            )
            resolution = CategoryResolution(
                query=category,
                matched=matched,
                confidence="partial" if matched else "none",
            )
            if matched:
                results = [_VELA]
        return SearchResult(
    ...
    async def list_categories(self) -> list[CatalogCategoryDTO]:
        if not self.available:
            raise CatalogUnavailableError("snapshot ausente")
        return [
            CatalogCategoryDTO("velas-aromaticas", "Velas Aromáticas", 1),
            CatalogCategoryDTO("zodiacal", "Zodiacal", 1),
        ]
...
async def test_search_returns_closed_lists_cop_price_and_clamps_limit() -> None:
    ...
    vela = out["results"][0]
    assert (
        vela["handle"] == "vela-lavanda"
        and vela["price"] == "35000"
        and vela["currency"] == "cop"
    )
    ...
    assert zod["variants"] == [
        {"id": "var_leo", "title": "Leo"},
        {"id": "var_aries", "title": "Aries"},
    ]
...
@pytest.mark.asyncio
async def test_search_reports_how_the_category_resolved_and_the_closed_list_when_it_did_not() -> (
    None
):
    cat = _FakeCatalog()
...
def test_fixtures_are_frozen_dtos() -> (
    None
):  # sanity: el fake usa los DTOs reales del port
    ...
    class _Down(_FakeCatalog):
        async def search(self, q, *, limit=10, category=None):
            raise CatalogUnavailableError(
                "snapshot not found at /app/hubara_vault/catalog/snapshot.json"
            )
