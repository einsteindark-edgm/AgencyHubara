...
El timeout normal de ghosting Sales son 60s. Cuando recién enviamos un
WhatsApp Flow nativo, el cliente necesita 1-3 minutos para llenar el form.
Si ghosting se dispara mientras tanto, arranca remarketing → claim_routing
pisa active_route → el nfm_reply se rutea a remarketing y el cliente queda
atascado.
...
@pytest.mark.asyncio
async def test_default_60s_when_no_metadata_file(vault):
    """Sesión nueva sin metadata.json → 60s normal."""
    result = await read_idle_timeout_seconds_activity("wa_nonexistent")
    assert result == 60


@pytest.mark.asyncio
async def test_default_60s_when_no_flag(vault):
    """Metadata existe pero sin el flag → 60s normal."""
...
@pytest.mark.asyncio
async def test_default_60s_when_flag_is_not_int(vault):
    """Flag con tipo inválido (string, None) → 60s normal — defensivo."""
    _write_metadata(
...
@pytest.mark.asyncio
async def test_default_60s_when_flag_expired(vault):
    """Flag con > 10 min de antigüedad → 60s (cliente ya no completó)."""
...
@pytest.mark.asyncio
async def test_default_60s_when_flag_in_future(vault):
    """Flag con timestamp futuro (clock skew, bug) → 60s normal — defensivo."""
...
@pytest.mark.asyncio
async def test_extended_timeout_floor_is_60s_even_near_cap(vault):
    """Si el flag está a 9.99 min de antigüedad (filo del cap), igual
    devolver mínimo 60s — el workflow no debe quedarse con timeouts de 1s
    ...
    assert result == 60  # piso mínimo aplicado
...
async def test_bad_metadata_json_returns_default(vault):
    """metadata.json corrupto → 60s normal, no crash."""
