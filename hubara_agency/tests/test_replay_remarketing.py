"""Replay test del workflow de Remarketing (F6.2 / ADR-005).

Carga la history sintetica capturada por `tests/fixtures/generate_fixtures.py`
y la re-ejecuta contra el codigo actual de `RemarketingSessionWorkflow`. Si la
...
El primer activity event de la fixture debe ser
`bootstrap_remarketing_session_activity` (PR #4 / F6.1).
...
async def test_remarketing_session_replay_does_not_diverge() -> None:
    history = WorkflowHistory.from_json(
        "test-remarketing", FIXTURE.read_text(encoding="utf-8")
    )
    replayer = Replayer(workflows=[RemarketingSessionWorkflow])
