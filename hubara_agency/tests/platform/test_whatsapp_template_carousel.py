...
Una campaña puede llevar 2..10 tarjetas. Dos sabores de Meta: **product
cards** (el número tiene catálogo conectado: foto y precio salen del
catálogo, botón Ver con carrito nativo — el que usa la campaña) y **media
cards** (foto por media_id + cuerpo + quick_reply `ref: HUB-<sku>`). Meta fija
la cantidad AL CREAR la plantilla, así que hay una plantilla por cantidad.

Contrato:
  * El catálogo declara `carousel_cards: n` (y un rango expande n entradas)
    + `carousel_kind`.
  * El builder emite `body` + `carousel.cards[]` según el sabor y se niega si
    la cantidad no matchea, si falta el producto/foto de una tarjeta o si se
    pasan tarjetas a una plantilla sin carrusel.
...
def _product_cards(n: int) -> list[CarouselCard]:
    return [
        CarouselCard(
            body_text=f"Producto {i} · $45.000",
            product_retailer_id=f"HUB-P{i}",
            catalog_id="868000000000000",
        )
        for i in range(n)
    ]
...
def test_catalog_parses_carousel_kind_and_rejects_unknown():
    entry = _entry(name="c", waba_template_name="c", carousel_cards=2, carousel_kind="product")
    entry.pop("carousel_cards_range")
    assert _build_registry_from_dict({"templates": [entry]})["c"].carousel_kind == "product"
    assert _build_registry_from_dict({"templates": [_entry()]})["carousel_v1_2"].carousel_kind == "media"
    with pytest.raises(ValueError, match="carousel_kind"):
        _build_registry_from_dict({"templates": [{**entry, "carousel_kind": "video"}]})
...
def test_real_catalog_has_campaign_carousel_for_2_to_10_cards():
    ...
    for n in range(2, 11):
        ...
        # El número del bot tiene catálogo conectado → product cards.
        assert spec.carousel_kind == "product"
...
def test_provisioning_defines_every_carousel_template_with_matching_cards():
    ...
    for n in range(2, 11):
        ...
        assert carousel["kind"] == "product"
        # Product card: botón SPM ("Ver") que abre el producto del catálogo.
        assert carousel["buttons"][0]["type"] == "SPM"
...
def _product_spec(n: int = 2):
    entry = _entry(carousel_kind="product")
    return _build_registry_from_dict({"templates": [entry]})[f"carousel_v1_{n}"]


def test_builder_emits_product_cards_with_catalog_reference():
    payload = build_template_message(
        "573001234567",
        _product_spec(2),
        {"greeting": "Hola", "message": "Mirá esto"},
        carousel_cards=_product_cards(2),
    )
    components = payload["template"]["components"]
    ...
    assert cards[1]["components"] == [
        {
            ...
            "parameters": [
                {
                    "type": "product",
                    "product": {"product_retailer_id": "HUB-P1", "catalog_id": "868000000000000"},
                }
            ],
        }
    ]
...
def test_builder_refuses_media_card_on_product_template_and_viceversa():
    with pytest.raises(ValueError, match="product_retailer_id"):
        build_template_message(
            "573001234567", _product_spec(2), {"greeting": "Hola", "message": "x"},
            carousel_cards=_cards(2),
        )
    with pytest.raises(ValueError, match="header_media_id"):
        build_template_message(
            "573001234567", _spec(2), {"greeting": "Hola", "message": "x"},
            carousel_cards=_product_cards(2),
        )
...
async def test_send_passes_cards_and_keeps_products_in_history(isolated_vault, monkeypatch):
    ...
    assert event["carousel"] == [
        {"body_text": "Producto 0 · $45.000", "product_retailer_id": "HUB-P0", "quick_reply_payload": None},
        {"body_text": "Producto 1 · $45.000", "product_retailer_id": "HUB-P1", "quick_reply_payload": None},
    ]
...
async def test_same_text_with_different_cards_is_not_deduped(isolated_vault, monkeypatch):
    ...
    other = [
        CarouselCard(body_text="Otro", product_retailer_id="HUB-O", catalog_id="868000000000000"),
        CarouselCard(body_text="Otro 2", product_retailer_id="HUB-O2", catalog_id="868000000000000"),
    ]
    for cards in (_product_cards(2), other):
