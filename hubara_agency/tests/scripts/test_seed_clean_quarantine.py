...
def _vault(tmp_path: Path) -> Path:
    ...
    vault.mkdir()
    _session(vault, "wa_573000000901", {"seeded_test": True, "tag": "INTERESADO"},
             ["[seed] mensaje 0", "[seed] mensaje 1"])
    # Sembrada sin el marker (versión vieja del script): se reconoce por el texto.
    _session(vault, "wa_573000000902", {"tag": "INTERESADO"}, ["[seed] mensaje 0"])
...
def test_find_seed_sessions_reconoce_marker_y_texto_seed() -> None:
    ...
    assert found == ["wa_573000000901", "wa_573000000902"]
...
def test_dry_run_no_toca_nada() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert (vault / "wa_573000000901").exists()
...
def test_apply_mueve_a_cuarentena_y_los_readers_dejan_de_verlas() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        moved = result.target_dir / "wa_573000000901"
        assert (moved / "metadata.json").exists()
        assert (moved / "sessions" / "wa_573000000901.jsonl").exists()
        assert not (vault / "wa_573000000901").exists()
        assert not (vault / "wa_573000000902").exists()
