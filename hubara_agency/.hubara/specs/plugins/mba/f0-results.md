...
| Primer sync de la config a Meta (D2.2) | ✅ 2026-09-11 | `POST /api/mba/agents/sales/sync` desde el contenedor: 28/29 ok (9 skills, 10 FAQ, 7 UI skills, business_info, settings); connector + 9 tools omitidos (gate); 1 rechazo: `request-shipping-details` → "A flow_id is required when component_type is flow" (fix en el PR siguiente). Verificado por lectura en Meta: settings intactos (`rollout.enabled=false`, `ALLOWLISTED_ONLY`, handoff CUSTOM). |
| Teléfono de prueba en la allowlist de Meta (D2.3) | ⏳ | +573125671604 ∈ `MBA_CUSTOMER_ALLOWLIST` ✅; en Meta: pendiente |
| Método de pago en el Billing Hub (WhatsApp Manager lo cuenta como tarea obligatoria) | ✅ 2026-09-11 (operador) | `GET /{business_id}/agent_budget` → 200 `{budgets: []}`; NO destrabó los connectors |
| Oído `standby` encendido en Hubara (`MBA_STANDBY_ENABLED=1`, Terraform) | ✅ 2026-09-11 | PR #273 + apply + deploy; el flag también gatea el apply del sync (`mba_disabled` con 0). Sin él Hubara no guarda el historial de lo que MBA conversa (D1.4) |
...
**2026-09-11 (noche) · sync aplicado; connectors siguen gateados.** Con `MBA_STANDBY_ENABLED=1` (PR #273) el sync entró: Meta tiene 9 skills, 10 FAQ, 7 UI skills, business_info completa; settings sin tocar. Con "Preguntas frecuentes" y "Aptitudes" completas en WhatsApp Manager, `GET agent_connectors` SIGUE en 400 (`fbtrace_id A7HACyxe-Kw12E6Xvgja_gR`). Descartado entonces que "finish onboarding" sean las tareas de configuración. Quedan dos hipótesis: (a) el gate se levanta con el método de pago o al habilitar el agente por primera vez (las 2 tareas restantes); (b) connectors es una capacidad que Meta habilita por entidad/WABA (rollout progresivo) → ticket a soporte de Meta con el `fbtrace_id`, el `entity_id` y el `agent_id`. Mientras tanto MBA puede probarse en el simulador con la config real (sin tools).

**2026-09-11 (noche, incidente) · `rollout.enabled` apareció en `true` sin acción de Hubara.** Tras #274 + dos syncs, `GET agent_config/settings` mostró `rollout.enabled: true`; antes del primer sync y justo después de él estaba en `false`. **Contención:** apagado por CLI en el acto (`PUT settings {rollout.enabled=false, ai_audience=ALLOWLISTED_ONLY}`) y releído. **Exposición: ninguna** — la audiencia seguía `ALLOWLISTED_ONLY` y la allowlist de Meta estaba vacía (nadie elegible). **Causa:** el historial de rollout de la tab está vacío (no fue Hubara) y un experimento controlado (PUT por separado de `followup`, `handoff` y `never_say_phrases`, releyendo tras cada uno) NO enciende el rollout → el sync queda descartado. **Causa confirmada por el operador (2026-09-11):** encendió el interruptor "Permite que el agente de IA responda" en WhatsApp Manager pensando que destrabaría los connectors. No destrabó nada (connectors siguieron en 400 con el agente encendido) y no expuso a nadie (allowlist vacía). Regla operativa: el encendido sale SOLO de la tab de rollout (dos pasos, con checks); la UI de Meta no valida nada. **Lección:** entre syncs nadie mira el drift → D4.3 (kill switch + alerta de drift) sube de prioridad; mientras tanto `whatsapp_provision.py mba-status` es el chequeo rápido y la tab de rollout muestra `drift` cuando el rollout está encendido sin que los checks pasen.

**2026-09-11 (noche) · sync 8/8 y una actualización fantasma.** Segundo sync tras #274: `request-shipping-details` creada con `flow_id` entero; un plan posterior la deja en `noop unchanged` (Meta devuelve `flow_id` en el GET → la verificación del reviewer pasa). WhatsApp Manager: 8 UI skills. Queda un `settings: update` en CADA plan: Meta guarda el followup apagado como `null` y el diff lo compara con nuestro `{enabled: false}` → PUT inútil por sync (inofensivo, verificado que no toca `rollout`), corregido en el PR siguiente.

**2026-09-11 (noche) · connectors: todas las hipótesis locales agotadas.** Con método de pago puesto y con el agente encendido un rato, `GET /{phone_id}/agent_connectors` sigue 400 (`fbtrace_id AxNY8R4BYMFO3x1mAmrsmvw`). Descartada la entidad: el OpenAPI dice que `entity_id` es el phone_number_id (solo `agent_budget` usa el business id); con el business id o el WABA responde 403 "Instagram connectors not enabled". El changelog y la página de capacidades no mencionan disponibilidad gradual. **Siguiente paso: ticket a soporte de Meta** (Business Help Center → soporte para desarrolladores) con: endpoint, `X-API-Version: 2.0.0`, `entity_id` (phone_number_id), `agent_id`, los `fbtrace_id` (`A7HACyxe-Kw12E6Xvgja_gR`, `AxNY8R4BYMFO3x1mAmrsmvw`), y que settings/skills/faq/ui-skills/allowlist/agent_test responden 200 para la misma entidad. Mientras tanto F0 puede avanzar con lo que no depende de tools: exp. 1 ✓, exp. 5 (tono) en el simulador, y exp. 3/4/6 (ráfagas, reacción, handoff nativo) con el rollout encendido para la lista cerrada.

**2026-09-11 · `flow_id` es un campo, no texto.** El OpenAPI de UI skills: `flow_id: integer`, "required when component_type is flow, and not supported otherwise"; el `PUT` no lo acepta (cambiarlo = borrar y recrear). Nuestro body lo llevaba solo dentro de la `instruction` → Meta rechazó `request-shipping-details`. Fix: campo `flow_id` en `agent.yaml` (`${META_FLOW_ID_SHIPPING}`), entero en el body, `replace` si cambia. **Verificar tras el próximo sync (gotcha #1):** que un segundo `sync/plan` deje `request-shipping-details` en `noop unchanged`; si sale `replace flow_id_changed` es que el `GET agent-ui-skills` de Meta no devuelve `flow_id` → pedirlo con `fields=`.
