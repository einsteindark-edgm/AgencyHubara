...
import {
  ...
  splitColumnChecks,
  stripLayout,
} from "@/shared/lib/trajectory-strip-layout";

interface Props {
  /** Modelo de vista ya armado por el dueño del dominio (columnas, bandas, checks). */
  model: StripModel;
 ...
 */
export function TrajectoryStrip({ model, selectedCheckId, onSelectCheck }: Props) {
  ...
  const stagesPresent = [...new Map(model.bands.map((b) => [b.stage, b])).values()];

  return (
    <figure className="m-0 flex min-w-0 flex-col gap-2">
      <ul className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-fg-muted" aria-label="Leyenda de la tira">
        {stagesPresent.map((band) => (
          <li key={band.stage ?? "sin-etapa"} className="inline-flex items-center gap-1.5">
            <span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ background: band.color }} aria-hidden="true" />
            {band.label}
          </li>
        ))}
            ...
            return (
              <g key={col.turn}>
                ...
                {(["cliente", "bot", "tools", "componentes", "estado", "guardas"] as const).map((laneId) => {
                  ...
                  if (laneId === "bot" && chips.length === 0 && col.botSilent) {
                    return (
                ...
                <text x={cx} y={height - 8} textAnchor="middle" fontSize={11} fill="var(--color-fg-faint)">
                  {`turno ${col.turn}`}
                  {col.triggerNote !== null ? ` · ${col.triggerNote}` : ""}
                </text>
              </g>
            );
          })}
