...
### 3.11 GraphAgents — caja EC2 dedicada (subsistema separado, full durable)

- **Qué es:** subsistema **aparte** del monorepo (`GraphAgents/`, no fusiona código; se une en "fase B" por un puente HTTP/execution-id). Agentes de análisis de **Meta Ads** con LangGraph sobre **AgentSpan** (runtime durable sobre Netflix Conductor). El **task graph durable es la columna vertebral** del desarrollo → corre el motor real (no LocalRuntime).
- **3 contenedores** (su `docker-compose.yml`): **`postgres`** (estado durable de Conductor) + **`agentspan`** (runtime JVM, `:6767`, heap ~1536m → imagen oficial `agentspan/server`) + **`graphagents`** (la app + explorer visual `:8900`; imagen propia a GHCR).
- **Dónde:** **una caja EC2 compartida** (`compute/modules/graphagents-instance`), t3.large (8GB, cómodo para la JVM; t3.medium es el mínimo justo), volumen gp3 40GB para el Postgres.
- **Pay-per-use (uso ocasional):** los análisis de ads se corren **muy de vez en cuando**, así que la caja vive **APAGADA por default** → solo pagás **~$3/mo de disco EBS** (para no perder el estado durable del Postgres). La prendés on-demand (`infra/scripts/graphagents_ctl.py start` o el deploy la prende solo), corrés el análisis, y **se auto-apaga sola** tras ~20 min de CPU idle (cron en la caja con permiso `ec2:StopInstances` sobre sí misma → imposible olvidarla prendida). El cómputo cuesta solo las horas que corre (~$0.08/h en t3.large = centavos por análisis). **SIN EIP** (IP dinámica resuelta por tag) para no pagar el cargo de EIP idle. Total realista: **~$3–6/mo** vs ~$60 always-on. AgentSpan no es serverless (es un server durable + Postgres), así que start/stop es la forma correcta de "pagar por uso" sin perder durabilidad.
- **Por qué self-host y no managed:** **AgentSpan es OSS/MIT, self-host por diseño** ("no account, no API key"); el único managed sería **Orkes Cloud** (Conductor managed), que es enterprise/contact-sales — sin floor transparente. Y está **off-critical-path** (analiza ads; si se cae, perdés análisis, no negocio) → **misma asimetría que SigNoz**: managed lo pagamos solo en el camino de ingresos (Temporal); acá self-host. Su riesgo de self-host (backups del Postgres) NO amenaza ingresos.
- **Transparencia runtime↔código:** el código del agente (el `StateGraph`) es **idéntico** en LocalRuntime vs AgentSpan — es un swap de vendor detrás del port `Runtime` (`GRAPHAGENTS_RUNTIME=local|agentspan`). Por eso arrancar minimal y crecer a durable no cuesta rework; acá vamos directo a durable.
- **Networking / seguridad:** `:6767` (agentspan) abierto **solo a las SGs de las cajas de app** (para el puente fase-B) + admin CIDR; `:8900` (explorer) **solo admin/túnel**, nunca público; **`:5432` (postgres) NUNCA expuesto**. Mismo criterio que SigNoz.
- **Secretos:** SSM `/graphagents/<KEY>` (`AGENTSPAN_MASTER_KEY`, `POSTGRES_PASSWORD`, `OPENAI/ANTHROPIC_API_KEY`, `META_ACCESS_TOKEN`, `META_AD_ACCOUNT_ID`, `GHCR_PULL_TOKEN`). La caja los lee con su instance profile. Deploy: `graphagents-deploy.yml`.
- **Upgrade de durabilidad:** el Postgres local en la caja es lo más barato (como el vault); si la durabilidad del estado de Conductor se vuelve crítica, moverlo a **RDS**. Orkes Cloud solo si superás el self-host (pedir quote).
