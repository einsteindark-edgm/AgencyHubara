...
10. **Invocar el pipeline EQUIVOCADO contamina el HU_ID → branch con espacio → refspec git inválido.** El pipeline PRINCIPAL `hu-hubara-pipeline` toma SOLO un issue URL o un HU_ID. El SUB-pipeline `hu-hubara-plugin-pipeline` toma `"<HU_ID> <plugin_id>"` (2 tokens). Si pasás el formato del sub-pipeline al principal (`archon workflow run hu-hubara-pipeline "HU-... agents_admin"`), `resolve-input` matchea `^HU-...` y `gen-hu-id` usa el string ENTERO como HU_ID — con el espacio y `agents_admin` adentro. Eso produce `branch = "hu/HU-... agents_admin"` (con espacio) → `git push origin HEAD:hu/... agents_admin` falla con `fatal: invalid refspec` → `commit-refinement`/`commit-plan` dan `node_error`, `classify-mode`+`rama-A` se skipean (cascade), y el run **limpia 30 min** (refina+planifica de cero bajo el HU_ID equivocado, corre final-validation/premortem) antes de cancelar engañosamente en premortem-blocked. **Fix**: `gen-hu-id` ahora valida `^HU-\d{8}-\d{4,6}-[a-z0-9][a-z0-9-]*$`; si falla emite `valid:'false'` + hint → `cancel-bad-hu-id` corta en segundos. `setup-branch` tiene `when: valid=='true'` (no crea `hu/null`). El sub-pipeline ya era seguro (split con `awk '{print $1/$2}'`). Caso visto: run 38d8223e.
11. **Premortem/review "blocked" FALSO — el gate lee task-result.yaml en el path equivocado.** `check-premortem-resolved` y `check-review-resolved` leían SOLO `$ARTIFACTS_DIR/task-result.yaml`. Pero el implementer, en resolución de premortem/review **multi-plugin**, ACTUALIZA el result existente en `hubara_agency/.hubara/results/<HU_ID>/feature-results/<plugin>/task-result.yaml` — NO escribe en `$ARTIFACTS_DIR`. Resultado: aunque el implementer resolvió todo (`promise: PREMORTEM_RESOLVED`, `fixes_deferred: []`), el gate no encontraba el archivo → `PR_RESULT_MISSING` → `cancel-on-premortem-blocked` (engañoso: dice "blocked" cuando en realidad está resuelto). El trabajo del implementer queda **stranded sin commitear** en el worktree `thread-XXX` (como el caso 5785857). **Fix**: ambos gates ahora buscan en AMBOS paths (`$ARTIFACTS_DIR` + `find feature-results -name task-result.yaml`), con precedencia broken>blocked>resolved y stdout single-line. Caso visto: run 894495e1 — premortem resuelto (10 backend tests + utf-8 + auth + ui-state) pero canceló como blocked; recuperé el trabajo del worktree (commit ba7f738) + arreglé los gates.

12. **Verificación visual: revisá `docker ps` ANTES de abrir el navegador.** El stack ya está desplegado en Docker — ahí están los puertos REALES. NO levantes un `vite`/backend a mano (se choca y perdés tiempo, me pasó 2 veces). Reales: **frontend `local-hubara-frontend` → http://localhost:5174** (Vite con HMR sobre bind-mount del source — editar `frontend_dashboard/src` se refleja en vivo, sin rebuild), **API `local-hubara-api` → :8000**, **Temporal UI → :8233**, Temporal gRPC → :7233. El `:5174` HMR-ea el checkout **`main`** (no un worktree); para previsualizar cambios de worktree, trabajá en main o re-apuntá el bind-mount (reinicia un servicio del stack vivo → pedí OK). Backend (`.py`) sí necesita rebuild (`cd hubara_agency && docker compose -f docker-compose.local.yml up -d --build <svc>`); frontend no. **Nunca uses :5173** (ahí squattea el Vite de Archon en IPv6 → split IPv4/::1 → previsualizás la app equivocada). Detalle en `frontend_dashboard/CLAUDE.md` §"Verificación visual".
