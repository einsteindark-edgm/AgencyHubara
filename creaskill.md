Necesito que crees un archivo en este proyecto con el siguiente contenido. 

Ruta exacta: .claude/skills/exoclaw-tech-refiner-archon/SKILL.md

Si la carpeta .claude/skills/exoclaw-tech-refiner-archon/ no existe, creala primero.

El contenido del archivo es una skill de Claude Code adaptada para invocarse desde 
nodos de workflow de Archon (https://archon.diy), no como slash command. Es la 
versión "Archon-only" de una skill standalone llamada exoclaw-tech-refiner que ya 
tengo instalada en ~/.claude/skills/exoclaw-tech-refiner/.

Acá va el contenido completo del SKILL.md (pegalo verbatim, no lo edites):
...
Cuando termines:
1. Confirmá la ruta donde quedó el archivo
2. Hacé `head -10` del archivo y mostrame el frontmatter para verificar
3. Hacé `wc -l` del archivo para confirmar el tamaño (~250-300 líneas esperadas)
4. NO modifiques nada del contenido — debe quedar idéntico al bloque que te pegué