...
- `IngestHandover` (`chats/agent/sales/use_cases/ingest_handover.py`, SDK-only, sin Temporal): regla pura `control_owner_for(new_owner_app_id, our_app_id=META_APP_ID)`: igual → `hubara`, distinto → `mba`, sin app id configurado o dueño nulo → NO se decide (WARNING, nada escrito: mejor un hueco visible que un dueño inventado). Persiste bajo `store.update` (flock): `control_owner`, `control_owner_app_id`, `control_owner_since_ms` (cuándo cambió el dueño), `control_owner_updated_at_ms` (último evento aunque repita dueño), `control_history[]` (últimos 50: `kind`, ambos app ids, `at_ms` de Meta, `received_at_ms`, `metadata`). Dedupe de reentregas por (`at_ms`, `new_owner_app_id`) contra el último de la historia. Lista cerrada como el oído `standby` (fuera → ERROR + nada escrito). Un primer handover de un cliente sin sesión la crea. NO toca `active_route` / `tag` (el eje bot/humano de Hubara es otro; el registry lo respeta en D1.6/D1.7).
- Config: `META_APP_ID` (`parse_app_id`: solo dígitos; placeholder = no configurado) — ya existía como parámetro SSM (`variables.tf`) y env del API en `docker-compose.base.yml`. Constantes `CONTROL_OWNER_MBA` / `CONTROL_OWNER_HUBARA` / `CONTROL_OWNERS` en `platform/constants.py` (cross-plugin: chats escribe, mba lee), re-exportadas por `src.sdk.runtime` (check en `test_mba_rollout_guard.py` + doc `01-fachada-sdk.md`).
