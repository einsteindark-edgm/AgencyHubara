...
Alcance restante: la lógica de cada tool (validar el body contra el schema de `agent.yaml`, resolver `customer_phone` → `session_key = wa_<phone>`, delegar por cast). Scoping por cliente: el macro `WHATSAPP_PHONE_NUMBER` llega como parámetro y se traduce a `session_key = wa_<phone>`. Rate limit básico. Cada tool delega a los use cases existentes vía cast:
