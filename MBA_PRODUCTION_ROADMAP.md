...
> **Estado:** En ejecución · **Actualizado:** 2026-09-07 (D1.2b: las 9 tools del connector con lógica real — las 4 de escritura por cast al contrato `session-actions@v1` de chats) · **Base estratégica:** `META_BUSINESS_AGENT_PLAN.md` (2026-07-02, por qué y cómo convivir con MBA).
...
> PRs mergeados a `main`: #228 (preview), #230 (requests literales), #233 (plugin `mba` + guard de routers públicos + forge), #234 (roadmap). #235 (D1.2a: tools de lectura + hardening del router público). D1.2b (tools de escritura por cast + `session-actions@v1` en chats + castkit modo service) en PR abierto; siguiente D1.3. Cada D siguiente es un PR chico desde `main`.
...
| `manage_conversation_tag` | POST | ✅ cast → `chats` `/tag` (reconciliación pendiente en §D1.3) | sí |
