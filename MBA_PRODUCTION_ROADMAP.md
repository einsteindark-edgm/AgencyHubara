...
> **Estado:** En ejecución · **Actualizado:** 2026-09-08 (D1.4: webhook `standby` — el oído de Hubara cuando MBA controla el hilo, al vault sin Temporal) · **Base estratégica:** `META_BUSINESS_AGENT_PLAN.md` (2026-07-02, por qué y cómo convivir con MBA).
...
Hecho:
- `sales.py` reparte por `field` (`parsers.webhook_fields`): `standby` → `parse_whatsapp_standby` (todos los `changes`, defensivo) → `IngestStandby` en background + `standby.statuses` al `IngestDeliveryStatus` de siempre; `messaging_handovers` → 200 + log (persistir `control_owner` es D1.5); `messages` o sin `field` → el path de siempre. Un `standby` NUNCA entra al ingest de Sales (`test_webhook_standby_gate.py`).
