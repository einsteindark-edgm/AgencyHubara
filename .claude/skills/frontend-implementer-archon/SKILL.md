...
- **META-GATE FAILURES ARE NEVER `status: passed`.** The meta-gate (`src/test/architecture/test_meta.arch.test.ts`) flags any modification to architecture-protected files (`.archon/workflows/`, `.claude/skills/frontend-*`, `src/test/architecture/`, `.dependency-cruiser.cjs`) on the current branch vs `origin/main`. If it fires:
    ...
    - **DO NOT set `ARCH_CHANGE_APPROVED=1`** in any command you run, neither to "check the rest of the test suite" nor to "see if the rest passes". That env var is a gate bypass reserved for the operator on an explicit architecture-change PR with an ADR. If you set it in your own bash, you are lying to the gate. Run `npm test` without bypass; if meta-gate fails, block.
