...
The following paths are protected: they encode the FSD architectural contract
...
every refinement that includes them in §3:
  ...
  - The `*_ALLOWLIST`, `CSS_FILE_ALLOWLIST`, `ARCHITECTURE_PROTECTED_PREFIXES`
    exports in `frontend_dashboard/src/test/architecture/helpers.ts`
...
Pre-flight check (run BEFORE Step 3 — DAG construction):
  ...
  3. If any match → REFUSE to decompose. Emit an empty plan-manifest.yaml with:
       ...
       notes: |
         ...
         they encode the FSD contract and modifying them silently would
...
The same rule applies on iteration: if `$LOOP_USER_INPUT` (human feedback)
asks the planner to add a task that edits a protected file, refuse with the
same manifest payload. The protected set exists precisely to make this
friction visible — bypassing it is a bug, not a feature.
