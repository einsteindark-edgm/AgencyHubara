...
Exploration budget (mandatory — applies across Step 1 and Step 1.5 combined)
- Max 20 file reads during context loading.
- Max 15 grep/find/glob invocations.
- Drive exploration by HU keywords (proper nouns, named features, verbs of action that appear in the HU text). Do NOT freely browse the repo.
...
At the end of context loading, the refinement document MUST contain a §0 section with these sub-lists:
  - Existing patterns reused / generalized — pattern name + `path:line` + one-line rationale (why this is the model to extend rather than reinvent). This is where you cite the previous feature whose pattern this HU should follow (e.g. the "asignar a humano" feature when the HU is about "intervenir conversación").
