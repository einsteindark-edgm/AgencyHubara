...
- **Causa raíz:** (1) un id `-preview` es un contrato de semanas, no de años, y se eligió para un camino que atiende clientes. (2) Los dos consumidores tragan el error: el failover solo se ejercita cuando DeepSeek YA falló (un fallback roto es invisible hasta el día que hace falta) y `customer_scoring/llm_summary.py` es never-raises (warning + resumen determinístico). (3) Medido en prod ese día (por SSM, dentro del contenedor litellm v1.86.2, con la key real): el id viejo sigue contestando 200 porque Google lo redirige en silencio al GA — `modelVersion=gemini-3.1-flash-lite` — aunque su propia tabla dice que un modelo apagado "is completely turned off". Ni roto ni sano: vivo por una cortesía sin fecha. (4) El bloque del alias traía además una clave vacía (`override_raw_request_params:`, que no existe en litellm) y un `api_key` duplicado desde el primer commit: PyYAML se queda con la última clave repetida sin avisar — el mismo typo sobre `model:` re-rutea un alias en silencio.
- **Fix aplicado:** alias → `gemini/gemini-3.5-flash-lite` (GA 2026-07-21, sin fecha de shutdown). Se descartó `gemini-3.1-flash-lite` porque ya tiene shutdown publicado (2027-05-07, reemplazo = el 3.5): migrar ahí era agendar esta misma tarea otra vez. Antes de elegir se probaron los candidatos con el MISMO litellm del proxy de prod: texto, tool call, ida y vuelta del tool call propio y — el caso real del failover — un historial con tool calls hechos por DeepSeek (sin thought signature de Gemini); los cuatro OK. Basura del YAML afuera (también en el ConfigMap de k8s) y `deploy/openlit/pricing.json` movido a la lista oficial ($0.30 in / $2.50 out por 1M, el output incluye thinking tokens). El deploy ya recrea `litellm` en cada corrida (`docker compose rm -sf litellm` + `up -d`), así que el merge alcanza: no hay paso manual.
- **Regla para el skill:** (1) en un `model_list` que sirve producción no entra un id `preview` / `exp` / `experimental` / `latest` / `beta` / `alpha` sin una excepción EXPLÍCITA, atada al id exacto y con VENCIMIENTO (una excepción sin fecha se pudre igual que el alias); un alias que atiende clientes — toda la cadena de failover del router — no admite excepción; y el id va ESCRITO en el YAML (por env o con comodín no se puede auditar). (2) "Responde 200" no prueba que el id esté vigente: mirá qué modelo contestó (`modelVersion`); un redirect silencioso es deuda con fecha desconocida, y con el precio y el comportamiento de OTRO modelo. (3) Todo camino never-raises necesita un guard que NO dependa de verlo fallar en vivo: el failover se prueba montando el config real en un `litellm.Router` con el primario caído, no esperando a que DeepSeek se caiga. (4) Re-apuntar un alias mueve sus DOS claves de precio (alias + id upstream), o el costo se le atribuye a otro modelo. (5) Antes de migrar algo "porque está roto", medí la premisa contra prod: acá la tarea decía "apagado" y la medición dijo "redirigido" — no cambió la decisión, pero sí la urgencia (no había incidente que apagar) y lo que quedó documentado.
- **Guard:** `tests/platform/test_litellm_model_ids_stable.py` — sobre los archivos REALES (compose + ConfigMap k8s): `::test_no_alias_points_to_a_short_lived_upstream_id`, `::test_every_exception_still_covers_a_real_alias_and_has_a_bounded_window` (única excepción: `gemini-pro-judge` → `gemini-3.1-pro-preview`, juez de evals, vence 2026-12-18), `::test_proxy_config_has_no_duplicate_keys`, `::test_no_alias_carries_a_null_litellm_param`, `::test_router_fallbacks_only_name_registered_aliases`, `::test_customer_summary_default_is_a_registered_alias`, `::test_priced_aliases_are_priced_as_the_model_they_serve_today`; y de COMPORTAMIENTO `::test_when_the_primary_fails_the_request_leaves_for_a_stable_gemini_id` (DeepSeek 500 → mira el id que sale hacia Google; control negativo: con el id viejo se pone rojo). El chequeo mismo tiene sus tests (`test_checker_*`): un guard que nunca se vio fallar no protege nada.
